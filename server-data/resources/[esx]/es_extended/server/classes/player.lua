...
function CreateExtendedPlayer(
	...
	local targetOverrides = Config.PlayerFunctionOverride
			and Core.PlayerFunctionOverrides[Config.PlayerFunctionOverride]
		or {}
	...
	function self.setCoords(coordinates)
		...
		local vector = type(coordinates) == "vector4" and coordinates
			or type(coordinates) == "vector3" and vector4(coordinates, 0.0)
			or vec(coordinates.x, coordinates.y, coordinates.z, coordinates.heading or 0.0)
	...
	function self.setAccountMoney(accountName, money, reason)
		...
		if not tonumber(money) then
			print(
				("[^1ERROR^7] Tried To Set Account ^5%s^0 For Player ^5%s^0 To An Invalid Number -> ^5%s^7"):format(
					accountName,
					self.playerId,
					money
				)
		...
		if money >= 0 then
			...
			if account then
				...
				print(
					("[^1ERROR^7] Tried To Set Invalid Account ^5%s^0 For Player ^5%s^0!"):format(
						accountName,
						self.playerId
					)
				)
	...
	function self.addAccountMoney(accountName, money, reason)
		...
		if money > 0 then
			...
			if account then
				...
				print(
					("[^1ERROR^7] Tried To Set Add To Invalid Account ^5%s^0 For Player ^5%s^0!"):format(
						accountName,
						self.playerId
					)
				)
		...
		if money > 0 then
			...
			if account then
				...
				if self.accounts[account.index].money - money > self.accounts[account.index].money then
					print(
						("[^1ERROR^7] Tried To Underflow Account ^5%s^0 For Player ^5%s^0!"):format(
							accountName,
							self.playerId
						)
					)
	...
	function self.removeInventoryItem(itemName, count)
		...
		if item then
			...
			if count > 0 then
				...
				print(
					("[^1ERROR^7] Player ID:^5%s Tried remove a Invalid count -> %s of %s"):format(
						self.playerId,
						count,
						itemName
					)
	...
	function self.setJob(newJob, grade)
		...
		if ESX.DoesJobExist(newJob, grade) then
			...
			print(
				("[es_extended] [^3WARNING^7] Ignoring invalid ^5.setJob()^7 usage for ID: ^5%s^7, Job: ^5%s^7"):format(
					self.source,
					job
				)
			)
	...
	function self.getMeta(index, subIndex)
		...
		if subIndex and type(metaData) == "table" then
			...
			if _type == "table" then
				...
				for i = 1, #subIndex do
					...
					if type(key) == "string" then
						...
						print(
							("[^1ERROR^7] xPlayer.getMeta subIndex should be ^5string^7 or ^5table^7! that contains ^5string^7, received ^5%s^7!, skipping..."):format(
								type(key)
							)
						)
					end
			...
			return print(
				("[^1ERROR^7] xPlayer.getMeta subIndex should be ^5string^7 or ^5table^7!, received ^5%s^7!"):format(
					_type
				)
			)
		end
	...
	function self.setMeta(index, value, subValue)
		...
		if not subValue then
			if _type ~= "number" and _type ~= "string" and _type ~= "table" then
				return print(
					("[^1ERROR^7] xPlayer.setMeta ^5%s^7 should be ^5number^7 or ^5string^7 or ^5table^7!"):format(
						value
					)
				)
			end
			...
			if _type ~= "string" then
				return print(
					("[^1ERROR^7] xPlayer.setMeta ^5value^7 should be ^5string^7 as a subIndex!"):format(value)
				)
			end
	...
	function self.clearMeta(index, subValues)
		...
		local metaData = self.metadata[index]
		if metaData == nil then
			return Config.EnableDebug and print(("[^1ERROR^7] xPlayer.clearMeta ^5%s^7 does not exist!"):format(index))
				or nil
		end
		...
		elseif type(subValues) == "string" then
			...
			if type(metaData) == "table" then
				...
				return print(
					("[^1ERROR^7] xPlayer.clearMeta ^5%s^7 is not a table! Cannot clear subValue ^5%s^7."):format(
						index,
						subValues
					)
				)
		...
		elseif type(subValues) == "table" then
			...
			for i = 1, #subValues do
				...
				if type(subValue) == "string" then
					...
					print(
						("[^1ERROR^7] xPlayer.clearMeta subValues should contain ^5string^7, received ^5%s^7, skipping..."):format(
							type(subValue)
						)
					)
				end
			...
			return print(
				("[^1ERROR^7] xPlayer.clearMeta ^5subValues^7 should be ^5string^7 or ^5table^7, received ^5%s^7!"):format(
					type(subValues)
				)
			)
		end
