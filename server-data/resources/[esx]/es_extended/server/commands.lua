...
local upgrades = Config.SpawnVehMaxUpgrades
		and {
			plate = "ADMINCAR",
			modEngine = 3,
			modBrakes = 2,
...
ESX.RegisterCommand(
	...
	function(xPlayer, args)
		...
		local Vehicles =
			ESX.OneSync.GetVehiclesInArea(GetEntityCoords(GetPlayerPed(xPlayer.source)), tonumber(args.radius) or 5.0)
		for i = 1, #Vehicles do
	...
	ESX.RegisterCommand(
					...
					if args.playerId.hasWeaponComponent(args.weaponName, args.componentName) then
						...
						if Config.AdminLogging then
							ESX.DiscordLogFields(
								"UserActions",
								"Give Weapon Component /giveweaponcomponent Triggered!",
								"pink",
								{
									{
										name = "Player",
										value = xPlayer and xPlayer.name or "Server Console",
										inline = true,
									},
									{ name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
									{ name = "Target", value = args.playerId.name, inline = true },
		...
		true,
		{
			...
			arguments = {
				...
				{
					name = "componentName",
					help = TranslateCap("command_giveweaponcomponent_component"),
					type = "string",
				},
			},
		}
...
ESX.RegisterCommand("job", { "user", "admin" }, function(xPlayer, _, _)
	print(
		("%s, your job is: ^5%s^0 - ^5%s^0"):format(
			xPlayer.getName(),
			xPlayer.getJob().name,
...
ESX.RegisterCommand("info", { "user", "admin" }, function(xPlayer)
	...
	print(
		("^2ID: ^5%s^0 | ^2Name: ^5%s^0 | ^2Group: ^5%s^0 | ^2Job: ^5%s^0"):format(
			xPlayer.source,
			xPlayer.getName(),
			xPlayer.getGroup(),
			job
