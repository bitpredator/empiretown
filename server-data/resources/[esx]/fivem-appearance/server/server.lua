...
AddEventHandler("fivem-appearance:saveOutfit", function(name, pedModel, pedComponents, pedProps)
	...
	local xPlayer = ESX.GetPlayerFromId(source)
	MySQL.insert.await(
		"INSERT INTO outfits (identifier, name, ped, components, props) VALUES (?, ?, ?, ?, ?)",
		{ xPlayer.identifier, name, json.encode(pedModel), json.encode(pedComponents), json.encode(pedProps) }
	)
end)
