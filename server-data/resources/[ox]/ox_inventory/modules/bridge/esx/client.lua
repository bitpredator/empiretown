...
function client.setPlayerStatus(values)
    for name, value in pairs(values) do
        if value > 0 then
            TriggerEvent("bpt_status:add", name, value)
        else
            TriggerEvent("bpt_status:remove", name, -value)
        end
    end
end
...
AddEventHandler("esx:setPlayerData", function(key, value)
    if not PlayerData.loaded or GetInvokingResource() ~= "es_extended" then
        return
    end
...
RegisterNetEvent("bpt_policejob:handcuff", function()
...
RegisterNetEvent("bpt_policejob:unrestrain", function()
