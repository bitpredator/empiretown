		...
		elseif data.current.value == "vehicle_interaction" then
			...
			if DoesEntityExist(vehicle) then
				elements3[#elements3 + 1] =
					{ icon = "fas fa-car", title = TranslateCap("vehicle_info"), value = "vehicle_infos" }
				elements3[#elements3 + 1] =
					{ icon = "fas fa-car", title = TranslateCap("pick_lock"), value = "hijack_vehicle" }
...
function OpenFineCategoryMenu(player, category)
	...
	for k, fine in ipairs(fineList[category]) do
		elements[#elements + 1] = {
			...
			title = ('%s <span style="color:green;">%s</span>'):format(
				fine.label,
				TranslateCap("armory_item", ESX.Math.GroupDigits(fine.amount))
			),
			value = fine.id,
			amount = fine.amount,
	...
	ESX.OpenContext("right", elements, function(menu, element)
		...
		if Config.EnablePlayerManagement then
			TriggerServerEvent("bpt_billing:sendBill", GetPlayerServerId(player), "society_police",
				TranslateCap("fine_total", data.current.fineLabel), data.current.amount
			)
		else
			TriggerServerEvent("bpt_billing:sendBill", GetPlayerServerId(player), "",
				TranslateCap("fine_total", data.current.fineLabel), data.current.amount
			)
		end
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback("bpt_policejob:getVehicleInfos", function(retrivedInfo)
				...
				if not retrivedInfo.owner then
					elements[#elements + 1] =
						{ unselectable = true, icon = "fas fa-user", title = TranslateCap("owner_unknown") }
				else
					elements[#elements + 1] =
						{ unselectable = true, icon = "fas fa-user", title = TranslateCap("owner", retrivedInfo.owner) }
				end
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback("bpt_policejob:getOtherPlayerData", function(playerData)
		...
		ESX.OpenContext("right", elements, function(menu, element)
			...
			TriggerServerEvent("bpt_policejob:message",
				GetPlayerServerId(player),
				TranslateCap("license_revoked", data.current.label)
			)
		...
		for k, bill in ipairs(bills) do
			elements[#elements + 1] = {
				...
				title = ('%s - <span style="color:red;">%s</span>'):format(
					bill.label,
					TranslateCap("armory_item", ESX.Math.GroupDigits(bill.amount))
				),
				billId = bill.id,
			}
		end
	...
	for k, v in ipairs(Config.AuthorizedWeapons[ESX.PlayerData.job.grade_name]) do
				...
				if v.components[i] then
					...
					if hasComponent then
						label = ('%s: <span style="color:green;">%s</span>'):format(
							component.label,
							TranslateCap("armory_owned")
						)
					else
						if v.components[i] > 0 then
							label = ('%s: <span style="color:green;">%s</span>'):format(
								component.label,
								TranslateCap("armory_item", ESX.Math.GroupDigits(v.components[i]))
							)
						else
							label = ('%s: <span style="color:green;">%s</span>'):format(
								component.label,
								TranslateCap("armory_free")
							)
						end
					end
		...
		elseif hasWeapon and not v.components then
			...
			if v.price > 0 then
				label = ('%s: <span style="color:green;">%s</span>'):format(
					weapon.label,
					TranslateCap("armory_item", ESX.Math.GroupDigits(v.price))
				)
			else
...
ESX.RegisterInput(
	"police:quickactions",
	"(ESX PoliceJob) " .. TranslateCap("quick_actions"),
	"keyboard",
	"F6",
	function()
