...
function DeleteJobVehicle()
    if Config.EnableSocietyOwnedVehicles then
        ...
        if IsInAuthorizedVehicle() then
            ...
            ESX.ShowNotification(TranslateCap("only_fisherman"))
        end
    end
end
...
function OpenFishermanActionsMenu()
    local elements = {
        { unselectable = true, icon = "fas fa-fisherman", title = TranslateCap("fisherman") },
    ...
    ESX.OpenContext("right", elements, function(_, element)
        ...
        CurrentAction = "fisherman_actions_menu"
        CurrentActionMsg = TranslateCap("press_to_open")
        CurrentActionData = {}
    end)
end

function OpenMobileFishermanActionsMenu()
    local elements = {
        { unselectable = true, icon = "fas fa-fisherman", title = TranslateCap("fisherman") },
            ...
            ESX.OpenContext("right", elements2, function(menu2)
                ...
                if amount == nil then
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        TriggerServerEvent("bpt_billing:sendBill", GetPlayerServerId(closestPlayer), "society_fisherman", "Fisherman", amount)
...
AddEventHandler("bpt_fishermanjob:hasEnteredMarker", function(zone)
    ...
    elseif zone == "FishermanActions" then
        CurrentAction = "fisherman_actions_menu"
        CurrentActionMsg = TranslateCap("press_to_open")
        CurrentActionData = {}
...
-- Create Blips
CreateThread(function()
    local blip = AddBlipForCoord(Config.Zones.FishermanActions.Pos.x, Config.Zones.FishermanActions.Pos.y, Config.Zones.FishermanActions.Pos.z)
    ...
    AddTextComponentSubstringPlayerName(TranslateCap("blip_fisherman"))
...
CreateThread(function()
    while true do
        local sleep = 1500
        if ESX.PlayerData.job and ESX.PlayerData.job.name == "fisherman" then
            ...
            if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
                ...
                TriggerEvent("bpt_fishermanjob:hasEnteredMarker", currentZone)
            end

            if not isInMarker and HasAlreadyEnteredMarker then
                HasAlreadyEnteredMarker = false
                TriggerEvent("bpt_fishermanjob:hasExitedMarker", LastZone)
            end
        end
        Wait(sleep)
    end
...
CreateThread(function()
    while true do
        ...
        if CurrentAction and not ESX.PlayerData.dead then
            ...
            if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == "fisherman" then
                if CurrentAction == "fisherman_actions_menu" then
                    OpenFishermanActionsMenu()
...
RegisterCommand("fishermanmenu", function()
    if not ESX.PlayerData.dead and Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.name == "fisherman" then
        OpenMobileFishermanActionsMenu()
    end
end, false)

RegisterKeyMapping("fishermanmenu", "Open Fisherman Menu", "keyboard", "f6")
