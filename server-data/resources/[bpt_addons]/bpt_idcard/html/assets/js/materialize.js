...
$jscomp.initSymbolIterator = function() {
	$jscomp.initSymbol(); var e = $jscomp.global.Symbol.iterator;
...
$jscomp.iteratorPrototype = function(e) {
	$jscomp.initSymbolIterator(); e = { next: e };
...
(function(r) {
	...
	function e(a) {
		...
		var d = C(a),
			b; for (b in a) {
			d[b] = Object.prototype.hasOwnProperty.call(c, b) ? c[b] : a[b];
		} return d;
	} function z(a, c) {
		var d = C(a),
			b; for (b in c) {
		...
		a = parseInt(d[1]) / 360; var b = parseInt(d[2]) / 100,
			f = parseInt(d[3]) / 100,
			d = d[4] || 1; if (b == 0) {
			f = b = a = f;
		}
		...
		if (h.dom(a) && u(W, c)) return 'transform'; if (h.dom(a) && (a.getAttribute(c) || h.svg(a) && a[c])) return 'attribute';
		if (h.dom(a) && c !== 'transform' && E(a, c)) return 'css';
		if (a[c] != null) return 'object';
	} function X(a, c) {
		var d = V(c),
			d = c.indexOf('scale') > -1 ? 1 : 0 + d; a = a.style.transform;
		if (!a) return d; for (var b = [], f = [], n = [], k = /(\w+)\((.+?)\)/g; b = k.exec(a);) {
		...
		else {f = (f = y(b)) ? b.substr(0, b.length - f.length) : b, b = c && !/\s/g.test(b) ? f + c : f;} b += '';
		...
		if (h.arr(a)) {
			...
			b = b ? 0 : c.delay; a = h.obj(a) && !h.pth(a) ? a : { value: a };
			h.und(a.delay) && (a.delay = b); return a;
		}).map(function(a) {
			return z(a, d);
		});
	} function ba(a, c) {
		...
		var d; return a.tweens.map(function(b) {
			b = ba(b, c); var f = b.value,
				...
				k = h.arr(f) ? f[0] : k,
				w = L(h.arr(f) ? f[1] : f, k),
				e = y(w) || y(k) || y(e); b.from = O(k, e);
			b.to = O(w, e); b.start = d ? d.end : a.offset;
			b.end = b.start + b.delay + b.duration; b.easing = ca(b.easing);
			b.elasticity = (1E3 - Math.min(Math.max(b.elasticity, 1), 999)) / 1E3; b.isPath = h.pth(f);
			b.isColor = h.col(b.from.original); b.isColor && (b.round = 1);
			return d = b;
		});
	} function ea(a, c) {
		...
		function c() {
			...
			for (var b = 0, c = {}, d = g.animations, f = d.length; b < f;) {
				var e = d[b],
					...
					var x = void 0,
						x = l.to.numbers[t],
						q = l.from.numbers[t],
						x = l.isPath ? Y(l.value, w * x) : q + w * (x - q); p && (l.isColor && t > 2 || (x = Math.round(x * p) / p));
					n.push(x);
				} if (l = h.length) {
					for (m = h[0], w = 0; w < l; w++) {
				...
				else {m = n[0];} ha[e.type](k.target, e.property, m, c, k.id);
				e.currentValue = m; b++;
			...
			if (q > n && q < k) b(q);
			else if (q <= n && r !== 0 && (b(0), x && e()), q >= k && r !== k || !k) b(k), x || e(); f('update');
		...
		var h,
			...
			g.progress = 0; g.paused = !0;
			g.began = !1; g.completed = !1;
			g.reversed = a === 'reverse';
			g.remaining = a === 'alternate' && c === 1 ? 2 : c; b(0);
			...
			var a = v.indexOf(g); a > -1 && v.splice(a, 1);
			g.paused = !0;
		}; g.play = function() {
			g.paused && (g.paused = !1, t = 0, l = d(g.currentTime), v.push(g), B || ia());
		}; g.reverse = function() {
			g.reversed = !g.reversed; t = 0;
			l = d(g.currentTime);
		}; g.restart = function() {
			g.pause();
			g.reset(); g.play();
		}; g.finished = m;
		g.reset(); g.autoplay && g.play();
		return g;
			...
			function a(a, d, b) {
				...
				if (c >= 0 && c <= 1 && b >= 0 && b <= 1) {
					var e = new Float32Array(11); if (c !== d || b !== f) {
						...
						if (c === d && b === f) return k; if (k === 0) return 0;
						if (k === 1) return 1; for (var h = 0, l = 1; l !== 10 && e[l] <= k; ++l) {
							h += 0.1;
						} --l;
	...
	q.path = function(a, c) {
		...
		var c = N(a); a.setAttribute('stroke-dasharray', c);
		return c;
	}; q.bezier = A;
	q.easings = Q; q.timeline = function(a) {
		var c = q(a); c.pause();
		c.duration = 0; c.add = function(d) {
			c.children.forEach(function(a) {
				a.began = !0; a.completed = !0;
			}); m(d).forEach(function(b) {
				var d = z(b, D(S, a || {})); d.targets = d.targets || a.targets;
				b = c.duration; var e = d.offset;
				d.autoplay = !1; d.direction = c.direction;
				d.offset = h.und(e) ? b : L(e, b); c.began = !0;
				c.completed = !0; c.seek(d.offset);
				d = q(d); d.began = !0;
				d.completed = !0; d.duration > b && (c.duration = d.duration);
				c.children.push(d);
			}); c.seek(0);
			c.reset(); c.autoplay && c.restart();
			return c;
		}; return c;
	...
	var FormSelect = function(_Component20) {
		...
		_createClass(FormSelect, [{
			...
			value: function _toggleEntryFromArray(key) {
				var notAdded = !Object.prototype.hasOwnProperty.call(this._keysSelected, key);
				if (notAdded) {
