	...
	ESX.UI.Menu.Open("default", GetCurrentResourceName(), menuName, {
		...
		for k, v in pairs(Config.Menus) do
			if k == data.current.modType or isRimMod then
				if
					data.current.label == TranslateCap("by_default")
					or string.match(data.current.label, TranslateCap("installed"))
				then
					ESX.ShowNotification(TranslateCap("already_own", data.current.label))
					myCar = ESX.Game.GetVehicleProperties(vehicle)
					TriggerServerEvent(
						"bpt_lscustom:refreshOwnedVehicle",
						myCar,
...
function GetAction(data)
	...
	if
		data.value == "modSpeakers"
		or data.value == "modTrunk"
		or data.value == "modHydrolic"
		or data.value == "modEngineBlock"
		or data.value == "modAirFilter"
		or data.value == "modStruts"
		or data.value == "modTank"
	then
		...
		if data.value == k then
			...
			if v.modType then
				...
				elseif
					v.modType == "color1"
					or v.modType == "color2"
					or v.modType == "pearlescentColor"
					or v.modType == "wheelColor"
				then
				...
				elseif v.modType == 17 then -- TURBO
					...
					if currentMods[k] then
						_label = 'Turbo - <span style="color:cornflowerblue;">'
							.. TranslateCap("installed")
							.. "</span>"
					else
						_label = 'Turbo - <span style="color:green;">$'
							.. math.floor(vehiclePrice * v.price[1] / 100)
							.. " </span>"
					end
					elements[#elements + 1] = {
						label = _label,
				...
				if
					data.value == "primaryRespray"
					or data.value == "secondaryRespray"
					or data.value == "pearlescentRespray"
					or data.value == "modFrontWheelsColor"
				then
