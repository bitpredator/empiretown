...
function putGloves()
	...
	AttachEntityToEntity(
		...
		1,
		true
	) -- object is attached to right hand
	AttachEntityToEntity(
		gloveB,
		ped,
		GetPedBoneIndex(ped, 0xAB22),
		0.05,
		0.00,
		-0.04,
		00.0,
...
function spawnMarker(coords)
	...
	if centerRing < Config.DISTANCE and fightStatus ~= STATUS_STARTED then
		...
		DrawText3D(
			...
			Config.CENTER.y,
			Config.CENTER.z + 1.5,
			"Giocatori: ~r~" .. players .. "/2 \n ~w~Scommessa: ~r~" .. betAmount .. "$ ",
			0.8
		)
		local blueZone =
			GetDistanceBetweenCoords(coords, vector3(Config.BLUEZONE.x, Config.BLUEZONE.y, Config.BLUEZONE.z), true)
		local redZone =
			GetDistanceBetweenCoords(coords, vector3(Config.REDZONE.x, Config.REDZONE.y, Config.REDZONE.z), true)
		local betZone =
			GetDistanceBetweenCoords(coords, vector3(Config.BETZONE.x, Config.BETZONE.y, Config.BETZONE.z), true)

		if blueJoined == false then
			DrawText3D(
				Config.BLUEZONE.x,
				Config.BLUEZONE.y,
				Config.BLUEZONE.z + 1.5,
				"unisciti alla lotta [~b~E~w~]",
				0.4
			)
	...
	while true do
		...
		if showCountDown == true then
			DrawText3D(
				...
				Config.CENTER.y,
				Config.CENTER.z + 1.5,
				"La lotta inizia in: " .. actualCount,
				2.0
			)
