...
lib.versionCheck('bitpredator/bpt_doorlock')
lib.locale()
...
exports('editDoor', function(id, data)
	...
	if door then
		...
		MySQL.update('UPDATE bpt_doorlock SET name = ?, data = ? WHERE id = ?', { door.name, encodeData(door), id })
		TriggerClientEvent('bpt_doorlock:editDoorlock', -1, id, door)
	end
end)

local soundDirectory = Config.NativeAudio and 'audio/dlc_bptdoorlock/bptdoorlock' or 'web/build/sounds'
...
lib.callback.register('bpt_doorlock:getSounds', function()
	return sounds
end)
...
local function createDoor(id, door, name)
	...
	if double then
		for i = 1, 2 do
			double[i].hash = joaat(('bpt_door_%s_%s'):format(id, i))
		...
		door.hash = joaat(('bpt_door_%s'):format(id))
	end
	...
	if type(door.items?[1]) == 'string' then
		...
		door.items = items
		MySQL.update('UPDATE bpt_doorlock SET data = ? WHERE id = ?', { encodeData(door), id })
	end
...
local function isAuthorised(playerId, door, lockpick)
	...
	if player then
		...
		if authorised ~= nil and door.passcode and not door.characters and not door.groups then
			authorised = door.passcode == lib.callback.await('bpt_doorlock:inputPassCode', playerId)
		end
	end
...
local sql = LoadResourceFile(cache.resource, 'sql/bpt_doorlock.sql')
...
MySQL.ready(function()
	...
	local response = MySQL.query.await('SELECT `id`, `name`, `data` FROM `bpt_doorlock`')
...
local function setDoorState(id, state, lockpick)
	...
	if door then
		...
		if authorised then
			door.state = state
			TriggerClientEvent('bpt_doorlock:setState', -1, id, state, source)
			...
			if door.autolock and state == 0 then
				SetTimeout(door.autolock * 1000, function()
					if door.state ~= 1 then
						...
						TriggerClientEvent('bpt_doorlock:setState', -1, id, door.state)
						TriggerEvent('bpt_doorlock:stateChanged', nil, door.id, door.state == 1)
					end
				end)
			end

			TriggerEvent('bpt_doorlock:stateChanged', source, door.id, state == 1, type(authorised) == 'string' and authorised)
...
RegisterNetEvent('bpt_doorlock:setState', setDoorState)
exports('setDoorState', setDoorState)

lib.callback.register('bpt_doorlock:getDoors', function()
...
RegisterNetEvent('bpt_doorlock:editDoorlock', function(id, data)
	if IsPlayerAceAllowed(source, 'command.doorlock') then
		...
		if id then
			if data then
				MySQL.update('UPDATE bpt_doorlock SET name = ?, data = ? WHERE id = ?', { data.name, encodeData(data), id })
			else
				MySQL.update('DELETE FROM bpt_doorlock WHERE id = ?', { id })
			end

			doors[id] = data
			TriggerClientEvent('bpt_doorlock:editDoorlock', -1, id, data)
		else
			local insertId = MySQL.insert.await('INSERT INTO bpt_doorlock (name, data) VALUES (?, ?)', { data.name, encodeData(data) })
			local door = createDoor(insertId, data, data.name)

			TriggerClientEvent('bpt_doorlock:setState', -1, door.id, door.state, false, door)
		end
	end
end)

RegisterNetEvent('bpt_doorlock:breakLockpick', function()
...
lib.addCommand('doorlock', {
	...
	TriggerClientEvent('bpt_doorlock:triggeredCommand', source, args.closest)
end)