...
function DeleteJobVehicle()
    if Config.EnableSocietyOwnedVehicles then
        ...
        if IsInAuthorizedVehicle() then
            ...
            ESX.ShowNotification(TranslateCap("only_dustman"))
        end
    end
end
...
function OpenDustmanActionsMenu()
    local elements = {
        { unselectable = true, icon = "fas fa-dustman", title = TranslateCap("dustman") },
            ...
            ESX.OpenContext("right", elements2, function(menu2)
                ...
                if amount == nil then
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        TriggerServerEvent("bpt_billing:sendBill", GetPlayerServerId(closestPlayer), "society_dustman", "Dustman", amount)
...
AddEventHandler("bpt_dustmanjob:hasEnteredMarker", function(zone)
    ...
    elseif zone == "DustmanActions" then
        CurrentAction = "dustman_actions_menu"
        CurrentActionMsg = TranslateCap("press_to_open")
        CurrentActionData = {}
...
-- Create Blips
CreateThread(function()
    local blip = AddBlipForCoord(Config.Zones.DustmanActions.Pos.x, Config.Zones.DustmanActions.Pos.y, Config.Zones.DustmanActions.Pos.z)
    ...
    AddTextComponentSubstringPlayerName(TranslateCap("blip_dustman"))
...
CreateThread(function()
    while true do
        local sleep = 1500
        if ESX.PlayerData.job and ESX.PlayerData.job.name == "dustman" then
            ...
            if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
                ...
                TriggerEvent("bpt_dustmanjob:hasEnteredMarker", currentZone)
            end

            if not isInMarker and HasAlreadyEnteredMarker then
                HasAlreadyEnteredMarker = false
                TriggerEvent("bpt_dustmanjob:hasExitedMarker", LastZone)
            end
        end
        Wait(sleep)
    end
...
CreateThread(function()
    while true do
        ...
        if CurrentAction and not ESX.PlayerData.dead then
            ...
            if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == "dustman" then
                if CurrentAction == "dustman_actions_menu" then
...
RegisterCommand("dustmanmenu", function()
    if not ESX.PlayerData.dead and Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.name == "dustman" then
        OpenMobileDustmanActionsMenu()
    end
end, false)
