...
TriggerEvent("bpt_society:registerSociety", "bennys", TranslateCap("society_bennys"), "society_bennys", "society_bennys", "society_bennys", {
    type = "public",
})

RegisterNetEvent("bpt_bennysjob:confiscatePlayerItem")
AddEventHandler("bpt_bennysjob:confiscatePlayerItem", function(target, itemType, itemName, amount)
    local source = source
    local sourceXPlayer = ESX.GetPlayerFromId(source)
    local targetXPlayer = ESX.GetPlayerFromId(target)

    if sourceXPlayer.job.name ~= "bennys" then
...
AddEventHandler("bpt_bennysjob:handcuff", function(target)
    local xPlayer = ESX.GetPlayerFromId(source)

    if xPlayer.job.name == "bennys" then
        TriggerClientEvent("bpt_bennysjob:handcuff", target)
    else
...
AddEventHandler("bpt_bennysjob:drag", function(target)
    local xPlayer = ESX.GetPlayerFromId(source)

    if xPlayer.job.name == "bennys" then
        TriggerClientEvent("bpt_bennysjob:drag", target, source)
    else
...
AddEventHandler("bpt_bennysjob:putInVehicle", function(target)
    local xPlayer = ESX.GetPlayerFromId(source)

    if xPlayer.job.name == "bennys" then
        TriggerClientEvent("bpt_bennysjob:putInVehicle", target)
    else
...
AddEventHandler("bpt_bennysjob:OutVehicle", function(target)
    local xPlayer = ESX.GetPlayerFromId(source)

    if xPlayer.job.name == "bennys" then
        TriggerClientEvent("bpt_bennysjob:OutVehicle", target)
    else
        print(("[^3WARNING^7] Player ^5%s^7 Attempted To Exploit Dragging Out Of Vehicle!"):format(xPlayer.source))
    end
end)

RegisterNetEvent("bpt_bennysjob:getStockItem")
AddEventHandler("bpt_bennysjob:getStockItem", function(itemName, count)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)

    TriggerEvent("bpt_addoninventory:getSharedInventory", "society_bennys", function(inventory)
...
RegisterNetEvent("bpt_bennysjob:putStockItems")
AddEventHandler("bpt_bennysjob:putStockItems", function(itemName, count)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("bpt_bennysjob:getOtherPlayerData", function(source, cb, target, notify)
...
ESX.RegisterServerCallback("bpt_bennysjob:getVehicleInfos", function(source, cb, plate)
    local retrivedInfo = {
        plate = plate,
    }
    if Config.EnableESXIdentity then
...
ESX.RegisterServerCallback("bpt_bennysjob:getbennysWeapons", function(source, cb)
    TriggerEvent("bpt_datastore:getSharedDataStore", "society_bennys", function(store)
...
ESX.RegisterServerCallback("bpt_bennysjob:addbennysWeapon", function(source, cb, weaponName, removeWeapon)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("bpt_bennysjob:removebennysWeapon", function(source, cb, weaponName)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("bpt_bennysjob:buyJobVehicle", function(source, cb, vehicleProps, type)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("bpt_bennysjob:storeNearbyVehicle", function(source, cb, plates)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("bpt_bennysjob:getStockItems", function(source, cb)
    TriggerEvent("bpt_addoninventory:getSharedInventory", "society_bennys", function(inventory)
        cb(inventory.items)
    end)
end)

ESX.RegisterServerCallback("bpt_bennysjob:getPlayerInventory", function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent("bpt_bennysjob:spawned")
AddEventHandler("bpt_bennysjob:spawned", function()
    local playerId = source
    local xPlayer = ESX.GetPlayerFromId(playerId)

    if xPlayer and xPlayer.job.name == "bennys" then
        Wait(5000)
        TriggerClientEvent("bpt_bennysjob:updateBlip", -1)
    end
end)

RegisterNetEvent("bpt_bennysjob:forceBlip")
AddEventHandler("bpt_bennysjob:forceBlip", function()
    for _, xPlayer in pairs(ESX.GetExtendedPlayers("job", "bennys")) do
        TriggerClientEvent("bpt_bennysjob:updateBlip", xPlayer.source)
    end
end)
