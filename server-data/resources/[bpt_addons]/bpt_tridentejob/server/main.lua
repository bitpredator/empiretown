...
if Config.EnableESXService then
    if Config.MaxInService ~= -1 then
        TriggerEvent("esx_service:activateService", "tridente", Config.MaxInService)
    end
end

TriggerEvent("bpt_society:registerSociety", "tridente", TranslateCap("society_tridente"), "society_tridente", "society_tridente", "society_tridente", {
    type = "public",
})

RegisterNetEvent("bpt_tridentejob:confiscatePlayerItem")
AddEventHandler("bpt_tridentejob:confiscatePlayerItem", function(target, itemType, itemName, amount)
    local source = source
    local sourceXPlayer = ESX.GetPlayerFromId(source)
    local targetXPlayer = ESX.GetPlayerFromId(target)

    if sourceXPlayer.job.name ~= "tridente" then
...
AddEventHandler("bpt_tridentejob:handcuff", function(target)
    local xPlayer = ESX.GetPlayerFromId(source)

    if xPlayer.job.name == "tridente" then
        TriggerClientEvent("bpt_tridentejob:handcuff", target)
    else
...
AddEventHandler("bpt_tridentejob:drag", function(target)
    local xPlayer = ESX.GetPlayerFromId(source)

    if xPlayer.job.name == "tridente" then
        TriggerClientEvent("bpt_tridentejob:drag", target, source)
    else
...
AddEventHandler("bpt_tridentejob:putInVehicle", function(target)
    local xPlayer = ESX.GetPlayerFromId(source)

    if xPlayer.job.name == "tridente" then
        TriggerClientEvent("bpt_tridentejob:putInVehicle", target)
    else
...
AddEventHandler("bpt_tridentejob:OutVehicle", function(target)
    local xPlayer = ESX.GetPlayerFromId(source)

    if xPlayer.job.name == "tridente" then
        TriggerClientEvent("bpt_tridentejob:OutVehicle", target)
    else
        print(("[^3WARNING^7] Player ^5%s^7 Attempted To Exploit Dragging Out Of Vehicle!"):format(xPlayer.source))
    end
end)

RegisterNetEvent("bpt_tridentejob:getStockItem")
AddEventHandler("bpt_tridentejob:getStockItem", function(itemName, count)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)

    TriggerEvent("bpt_addoninventory:getSharedInventory", "society_tridente", function(inventory)
...
RegisterNetEvent("bpt_tridentejob:putStockItems")
AddEventHandler("bpt_tridentejob:putStockItems", function(itemName, count)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("bpt_tridentejob:getOtherPlayerData", function(source, cb, target, notify)
...
local fineList = {}
ESX.RegisterServerCallback("bpt_tridentejob:getFineList", function(source, cb, category)
...
ESX.RegisterServerCallback("bpt_tridentejob:getVehicleInfos", function(source, cb, plate)
    local retrivedInfo = {
        plate = plate,
    }
    if Config.EnableESXIdentity then
...
ESX.RegisterServerCallback("bpt_tridentejob:getTridenteWeapons", function(source, cb)
    TriggerEvent("bpt_datastore:getSharedDataStore", "society_tridente", function(store)
...
ESX.RegisterServerCallback("bpt_tridentejob:addTridenteWeapon", function(source, cb, weaponName, removeWeapon)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("bpt_tridentejob:removeTridenteWeapon", function(source, cb, weaponName)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("bpt_tridentejob:buyJobVehicle", function(source, cb, vehicleProps, type)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("bpt_tridentejob:storeNearbyVehicle", function(source, cb, plates)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("bpt_tridentejob:getStockItems", function(source, cb)
    TriggerEvent("bpt_addoninventory:getSharedInventory", "society_tridente", function(inventory)
        cb(inventory.items)
    end)
end)

ESX.RegisterServerCallback("bpt_tridentejob:getPlayerInventory", function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent("bpt_tridentejob:spawned")
AddEventHandler("bpt_tridentejob:spawned", function()
    local playerId = source
    local xPlayer = ESX.GetPlayerFromId(playerId)

    if xPlayer and xPlayer.job.name == "tridente" then
        Wait(5000)
        TriggerClientEvent("bpt_tridentejob:updateBlip", -1)
    end
end)

RegisterNetEvent("bpt_tridentejob:forceBlip")
AddEventHandler("bpt_tridentejob:forceBlip", function()
    for _, xPlayer in pairs(ESX.GetExtendedPlayers("job", "tridente")) do
        TriggerClientEvent("bpt_tridentejob:updateBlip", xPlayer.source)
    end
end)
