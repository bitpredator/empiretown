	...
	while true do
		...
		if WarMenu.Begin("carRental") then
			...
			if WarMenu.Button("Rent Vehicle", price .. "$") then
				...
				TriggerServerEvent(
					"sqz_carrental:RentVehicle",
					vehiclesToRent[vehicleId],
					insurance,
					price,
					Config.RentalTimes[rentalTimeIndex],
					index
				)
				return
			end
...
AddEventHandler("sqz_carrental:SpawnVehicle", function(model, insurance, price, time, rentalIndex)
	...
	local vehicle =
		CreateVehicle(model, Config.Points[rentalIndex].SpawnPoint, GetEntityHeading(PlayerPedId()), true, false)
