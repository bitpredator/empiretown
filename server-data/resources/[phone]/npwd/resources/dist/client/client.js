...
(() => {
	...
	const init_deepMergeObjects = __esm({
		'../shared/deepMergeObjects.ts'() {
		},
	});

	// ../config.default.json
	let config_default_default;
	const init_config_default = __esm({
		'../config.default.json'() {
	...
	// ../typings/phone.ts
	const init_phone = __esm({
		'../typings/phone.ts'() {
		},
	});

	// ../typings/settings.ts
	const init_settings = __esm({
		'../typings/settings.ts'() {
		},
	});

	// client/settings/client-kvp.service.ts
	let _KvpService, KvpService, client_kvp_service_default;
	const init_client_kvp_service = __esm({
		'client/settings/client-kvp.service.ts'() {
			_KvpService = class {
				setKvp(key, value) {
	...
	let prop, propCreated, newPhoneProp;
	const init_functions = __esm({
		'client/functions.ts'() {
			init_settings();
			...
			newPhoneProp = () => __async(void 0, null, function* () {
				...
				let phoneModel;
				if (hasNPWDProps) {
					phoneModel = client_kvp_service_default.getKvpString('npwd-frame' /* NPWD_FRAME */) || 'prop_npwd_default';
				}
				else {
	...
	const init_animation_service = __esm({
		'client/animations/animation.service.ts'() {
			init_functions();
			init_fivem();
	...
	let animationService;
	const init_animation_controller = __esm({
		'client/animations/animation.controller.ts'() {
	...
	function togglePhone() {
		return __async(this, null, function* () {
			...
			if (!canAccess) {return;}
			if (global.isPhoneOpen) {return yield hidePhone();}
			yield showPhone();
		});
	}
	let exps, getCurrentGameTime, showPhone, hidePhone, checkHasPhone;
	const init_cl_main = __esm({
		'client/cl_main.ts'() {
			init_messages();
			init_phone();
			init_cl_config();
			...
			RegisterCommand(
				...
				() => __async(void 0, null, function* () {
					if (!global.isPhoneDisabled && !IsPauseMenuActive()) {yield togglePhone();}
				}),
				false,
			);
			RegisterCommand(
	...
	// ../typings/twitter.ts
	const init_twitter = __esm({
		'../typings/twitter.ts'() {
		},
	});

	// client/cl_twitter.ts
	const init_cl_twitter = __esm({
		'client/cl_twitter.ts'() {
			init_twitter();
			init_messages();
			init_cl_utils();
	...
	// ../typings/contact.ts
	const init_contact = __esm({
		'../typings/contact.ts'() {
		},
	});

	// client/cl_contacts.ts
	const init_cl_contacts = __esm({
		'client/cl_contacts.ts'() {
			init_contact();
			init_cl_utils();
	...
	const init_marketplace = __esm({
		'../typings/marketplace.ts'() {
		},
	});

	// client/cl_marketplace.ts
	const init_cl_marketplace = __esm({
		'client/cl_marketplace.ts'() {
			init_marketplace();
			init_cl_utils();
			init_messages();
	...
	// ../typings/notes.ts
	const init_notes = __esm({
		'../typings/notes.ts'() {
		},
	});

	// client/cl_notes.ts
	const init_cl_notes = __esm({
		'client/cl_notes.ts'() {
			init_notes();
			init_cl_utils();
	...
	// ../typings/photo.ts
	const init_photo = __esm({
		'../typings/photo.ts'() {
		},
	});

	// client/cl_photo.ts
	const require_cl_photo = __commonJS({
	...
	var init_cl_notifications = __esm({
		'client/cl_notifications.ts'() {
			...
			RegisterNuiCB('npwd:onNotificationConfirm', (notisId, cb) => {
				...
				if (!funcRef) {return console.log(`NPWD could not find any function ref for notification: ${notisId}`);}
				funcRef();
	...
	var require_cl_exports = __commonJS({
		'client/cl_exports.ts'(exports) {
			...
			exps2('setPhoneVisible', (bool) => __async(exports, null, function* () {
				...
				if (coercedType) {yield showPhone();}
				else {yield hidePhone();}
			}));
			...
			exps2('createSystemNotification', (dto) => {
				...
				if (dto.controls && !dto.keepOpen) {return console.log('Notification must be set to keepOpen in order to use notifcation actions');}
				if (!dto.controls && actionSet) {return console.log('Controls must be set to true in order to use notifcation actions');}
				if (dto.controls) {
	...
	var init_client_settings = __esm({
		'client/settings/client-settings.ts'() {
			...
			RegisterNuiCB('npwd:nuiSettingsUpdated' /* NUI_SETTINGS_UPDATED */, (cfg, cb) => {
				...
				client_kvp_service_default.setKvp('npwd-frame' /* NPWD_FRAME */, `prop_npwd_${frameName}`);
				cb({});
			});
