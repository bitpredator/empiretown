...
SetAudioSubmixOutputVolumes(radioEffectId, 0, 1.0 --[[ frontLeftVolume ]], 0.25 --[[ frontRightVolume ]], 0.0 --[[ rearLeftVolume ]], 0.0 --[[ rearRightVolume ]], 1.0 --[[ channel5Volume ]], 1.0 --[[ channel6Volume ]])
AddAudioSubmixOutput(radioEffectId, 0)
submixIndicies["radio"] = radioEffectId

local callEffectId = CreateAudioSubmix("Call")
SetAudioSubmixOutputVolumes(callEffectId, 1, 0.10 --[[ frontLeftVolume ]], 0.50 --[[ frontRightVolume ]], 0.0 --[[ rearLeftVolume ]], 0.0 --[[ rearRightVolume ]], 1.0 --[[ channel5Volume ]], 1.0 --[[ channel6Volume ]])
...
function playMicClicks(clickType)
    if micClicks ~= "true" then
        return logger.verbose("Not playing mic clicks because client has them disabled")
    end
...
if gameVersion == "redm" then
    CreateThread(function()
        while true do
            if
                IsControlJustPressed(0, 0xA5BDCD3C --[[ Right Bracket ]])
            then
                ExecuteCommand("cycleproximity")
            end
            if
                IsControlJustPressed(0, 0x430593AA --[[ Left Bracket ]])
            then
                ExecuteCommand("+radiotalk")
            elseif
                IsControlJustReleased(0, 0x430593AA --[[ Left Bracket ]])
            then
                ExecuteCommand("-radiotalk")
            end
