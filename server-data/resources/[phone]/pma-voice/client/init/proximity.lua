...
function addNearbyPlayers()
    ...
    if LocalPlayer.state.disableProximity then
        return
    end
...
CreateThread(function()
    ...
    TriggerEvent("chat:addSuggestion", "/muteply", "Mutes the player with the specified id", {
        ...
        { name = "duration", help = "(opt) mute duration in seconds (default: 900)" },
    })

    while true do
        -- Attesa connessione mumble
        while not MumbleIsConnected() do
            Wait(100)
        end

        -- Aggiornamento stato voce
        if GetConvarInt("voice_enableUi", 1) == 1 then
            local talking = (MumbleIsPlayerTalking(PlayerId()) == 1)

            if lastRadioStatus ~= radioPressed or lastTalkingStatus ~= talking then
                lastRadioStatus = radioPressed
                lastTalkingStatus = talking

                sendUIMessage({
                    usingRadio = radioPressed,
                    talking = talking,
                })
            end
        end

        -- Gestione modalità prossimità
        if voiceState == "proximity" then
            addNearbyPlayers()

            local camActive = GetConvarInt("voice_disableAutomaticListenerOnCamera", 0) ~= 1 and GetRenderingCam() ~= -1
            local isSpectating = NetworkIsInSpectatorMode() or camActive
