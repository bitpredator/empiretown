...
function serverlog(...)
    TriggerServerEvent('esx_uteknark2:log',...)
end

RegisterNetEvent('esx_uteknark2:make_toast')
AddEventHandler ('esx_uteknark2:make_toast', function(subject,message)
...
RegisterNetEvent('esx_uteknark2:do')
AddEventHandler ('esx_uteknark2:do', function(scenarioName, location)
...
RegisterNetEvent('esx_uteknark2:attempt_plant')
AddEventHandler ('esx_uteknark2:attempt_plant', function()
    local plantable, message, location, _, soil = getPlantingLocation()
    if plantable then
        TriggerServerEvent('esx_uteknark2:success_plant', location, soil)
                ...
                if closestDistance <= Config.Distance.Interact then
                    ...
                    if now >= lastAction + Config.ActionTime then
                        if IsDisabledControlJustPressed(0, 44) then
                            ...
                            TriggerServerEvent('esx_uteknark2:remove', closestPlant.id, myLocation)
                        else
                            if stage.interact then
                                interactHelp(closestPlant.stage, _U(stage.label))
                                if IsControlJustPressed(0, 38) then
                                    lastAction = now
                                    TriggerServerEvent('esx_uteknark2:frob', closestPlant.id, myLocation)
                                end
                            else
...
RegisterNetEvent('esx_uteknark2:toggle_debug')
AddEventHandler ('esx_uteknark2:toggle_debug', function()
...
RegisterNetEvent('esx_uteknark2:pyromaniac')
AddEventHandler ('esx_uteknark2:pyromaniac',function(location)
...
RegisterNetEvent('esx_uteknark2:groundmat')
AddEventHandler ('esx_uteknark2:groundmat', function()
    local plantable, message, where, normal, material = getPlantingLocation(true)
...
RegisterNetEvent('esx_uteknark2:test_forest')
AddEventHandler ('esx_uteknark2:test_forest',function(count, randomStage)
    ...
    TriggerServerEvent('esx_uteknark2:test_forest', forest)
end)
