...
function makeToast(target, subject, message)
    TriggerClientEvent('esx_uteknark2:make_toast', target, subject, message)
end
...
function doScenario(who, what, where)
    ...
    TriggerClientEvent('esx_uteknark2:do', who, what, where)
end

RegisterNetEvent('esx_uteknark2:success_plant')
AddEventHandler ('esx_uteknark2:success_plant', function(location, soil)
    local src = source
...
RegisterNetEvent('esx_uteknark2:log')
AddEventHandler ('esx_uteknark2:log',function(...)
    local src = source
    log(src,GetPlayerName(src),...)
end)

RegisterNetEvent('esx_uteknark2:test_forest')
AddEventHandler ('esx_uteknark2:test_forest',function(forest)
    local src = source
                ...
                ESX.RegisterUsableItem(Config.Items.Seed, function(source)
                    ...
                    if now > last + (Config.ActionTime/1000) then
                        if HasItem(source, Config.Items.Seed) then
                            TriggerClientEvent('esx_uteknark2:attempt_plant', source)
...
local commands = {
    debug = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark2:toggle_debug', source)
        end
    end,
    ...
    forest = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark2:test_forest', source, count, randomStage)
    ...
    groundmat = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark2:groundmat', source)
        end
    end,
    pyro = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark2:pyromaniac', source)
        end
    end,
}
