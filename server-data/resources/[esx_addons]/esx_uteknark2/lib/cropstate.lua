...
local cropstateMethods = {
    plant = function(instance, location, soil, stage)
        if onServer then
            stage = stage or 1
            MySQL.Async.insert("INSERT INTO `uteknark2` (`x`, `y`, `z`, `soil`, `stage`) VALUES (@x, @y, @z, @soil, @stage);",
            {
                ['@x'] = location.x,
                ['@y'] = location.y,
                ['@z'] = location.z,
            ...
            function(id)
                ...
                TriggerClientEvent('esx_uteknark2:planted',-1, id, location, stage)
    ...
    load = function(instance, callback)
        if onServer then
            verbose('Loading...')
            MySQL.Async.fetchAll("SELECT `id`, `stage`, UNIX_TIMESTAMP(`time`) AS `time`, `x`, `y`, `z`, `soil` FROM `uteknark2`;", 
            {},
            function(rows)
    ...
    update = function(instance, id, stage)
        ...
        if onServer then
            ...
            MySQL.Async.execute("UPDATE `uteknark2` SET `stage` = @stage WHERE `id` = @id LIMIT 1;",
            {
                ['@id'] = id,
                ['@stage'] = stage,
            }, function(_)
                TriggerClientEvent('esx_uteknark2:update', -1, id, stage)
    ...
    remove = function(instance, id, withPyro)
        ...
        if onServer then
            MySQL.Async.execute("DELETE FROM `uteknark2` WHERE `id` = @id LIMIT 1;",
            { ['@id'] = id },
            function()
                TriggerClientEvent('esx_uteknark2:removePlant', -1, id)
                if withPyro then
                    TriggerClientEvent('esx_uteknark2:pyromaniac', -1, location)
                end
    ...
    bulkData = function(instance, target)
        if onServer then
            ...
            TriggerClientEvent('esx_uteknark2:bulk_data', target, forest)
        else
            TriggerServerEvent('esx_uteknark2:request_data')
        end
    end,
}
...
if onServer then
    RegisterNetEvent('esx_uteknark2:request_data')
    AddEventHandler ('esx_uteknark2:request_data', function()
        cropstate:bulkData(source)
    end)
    
    RegisterNetEvent('esx_uteknark2:remove')
    AddEventHandler ('esx_uteknark2:remove', function(plantID, nearLocation)
        local src = source
        ...
        if plant then
            ...
            TriggerClientEvent('esx_uteknark2:remove', src, plantID)
        end
    end)

    RegisterNetEvent('esx_uteknark2:frob')
    AddEventHandler ('esx_uteknark2:frob', function(plantID, nearLocation)
        local src = source
        local plant = cropstate.index[plantID]
        if plant then
            local plantLocation = plant.bounds.location
    ...
    RegisterNetEvent('esx_uteknark2:bulk_data')
    AddEventHandler ('esx_uteknark2:bulk_data', function(forest)
    ...
    RegisterNetEvent('esx_uteknark2:planted')
    AddEventHandler ('esx_uteknark2:planted', function(id, location, stage)
        cropstate:import(id, location, stage)
    end)
    
    RegisterNetEvent('esx_uteknark2:update')
    AddEventHandler ('esx_uteknark2:update', function(plantID, stage)
        cropstate:update(plantID, stage)
    end)

    RegisterNetEvent('esx_uteknark2:removePlant')
    AddEventHandler ('esx_uteknark2:removePlant', function(plantID)
        cropstate:remove(plantID)
    end)
end
