		...
		cropstate.octree:searchSphereAsync(here, drawDistance, function(entry)
			if not entry.data.object and not entry.data.deleted then
				...
				if not HasModelLoaded(model) then
					...
					table.insert(
						activePlants,
						{ node = entry, object = weed, at = entry.bounds.location, stage = stage, id = entry.data.id }
					)
		...
		if #(location - myLocation) <= Config.Distance.Draw then
			CreateThread(function()
				...
				if not HasNamedPtfxAssetLoaded(Config.Burn.Effect) then
					...
					while
						not HasNamedPtfxAssetLoaded(Config.Burn.Collection)
						and GetGameTimer() <= begin + Config.Burn.Duration
					do
						Wait(0)
					end
				...
				local handle = StartParticleFxLoopedAtCoord(
					Config.Burn.Effect,
					location + Config.Burn.Offset,
					Config.Burn.Rotation,
					Config.Burn.Scale * 1.0,
					false,
					false,
					false
				)
