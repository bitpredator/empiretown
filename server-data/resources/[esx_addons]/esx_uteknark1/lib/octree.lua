...
local nodeMethods = {
	...
	split = function(instance)
		if not instance._data.nodes then
			...
			table.insert(
				instance._data.nodes,
				pOctree(vector3(pos.x + quarter.x, pos.y + quarter.x, pos.z + quarter.z), half, nextLevel)
			)
			table.insert(
				instance._data.nodes,
				pOctree(vector3(pos.x - quarter.x, pos.y + quarter.x, pos.z + quarter.z), half, nextLevel)
	...
	insert = function(instance, location, dimensions, data)
		...
		elseif type(dimensions) ~= "vector3" then
			error(
				"Second argument to :insert must be either a number (if cube) or a vector3 for dimetions, got "
					.. type(dimensions),
				2
			)
		end
