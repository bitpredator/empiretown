...
IR8.Database = {
	...
	GetTickets = function(admin, identifier)
		local rows = MySQL.query.await(
			"SELECT * FROM `" .. IR8.Config.DatabaseTables.Tickets .. "` WHERE identifier = ? ORDER BY id DESC",
			{
				identifier,
			}
		)
	...
	GetReplies = function(id)
		local rows = MySQL.query.await(
			"SELECT * FROM `" .. IR8.Config.DatabaseTables.Messages .. "` WHERE ticket_id = ? ORDER BY id DESC",
			{
				id,
			}
		)
	...
	CreateReply = function(identifier, name, data)
		local query = "INSERT INTO `"
			.. IR8.Config.DatabaseTables.Messages
			.. "`(ticket_id, name, message, identifier) VALUES(?, ?, ?, ?)"
	...
	CreateTicket = function(identifier, name, data)
		...
		local query = "INSERT INTO `"
			.. IR8.Config.DatabaseTables.Tickets
			.. "`(title, category, name, message, status, identifier, position) VALUES(?, ?, ?, ?, ?, ?, ?)"
		local res = MySQL.insert.await(query, {
			data.title,
			data.category,
			name,
