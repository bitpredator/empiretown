...
function UnJail()
    ...
    ESX.ShowNotification(TranslateCap("you_released"))
end
...
function InJail()
    ...
    CreateThread(function()
        while jailTime > 0 do
            jailTime = jailTime - 1
            ESX.ShowNotification(TranslateCap("few_minutes"))
                    ...
                    if DistanceCheck <= 1.5 then
                        if IsControlJustPressed(0, 38) then
                            if v["state"] then
                                ...
                                ESX.ShowNotification(TranslateCap("package_taken"))
                            end
                        end
                    end
                end
...
function PackPackage(packageId)
    ...
    while Packaging do
        ...
        if not IsPedUsingScenario(PlayerPedId(), "PROP_HUMAN_BUM_BIN") then
            ...
            ESX.ShowNotification(TranslateCap("canceled"))
            Packaging = false
        end
        ...
        if action == "jail_closest_player" then
            ...
            ESX.UI.Menu.Open("dialog", GetCurrentResourceName(), "jail_choose_time_menu", {
                ...
                if jailTime == nil then
                    ESX.ShowNotification(TranslateCap("time_minutes"))
                else
                    menu2.close()
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        ESX.UI.Menu.Open("dialog", GetCurrentResourceName(), "jail_choose_reason_menu", {
                            ...
                            if reason == nil then
                                ESX.ShowNotification(TranslateCap("put_something"))
                            else
                                menu3.close()
                                local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                                if closestPlayer == -1 or closestDistance > 3.0 then
        ...
        elseif action == "unjail_player" then
            ...
            ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
                if #playerArray == 0 then
                    ESX.ShowNotification(TranslateCap("jail_empty"))
                    return
                end

                for i = 1, #playerArray, 1 do
                    table.insert(elements, {
                        label = "Prisoner: " .. playerArray[i].name .. " | Jail Time: " .. playerArray[i].jailTime .. " minutes",
