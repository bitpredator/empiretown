    ...
    if xPlayer["job"]["name"] == "police" then
        ...
        if GetPlayerName(jailPlayer) ~= nil then
            if jailTime ~= nil then
                ...
                if args[3] ~= nil then
                    GetRPName(jailPlayer, function(Firstname, Lastname)
                        TriggerClientEvent("chat:addMessage", -1, {
                            args = {
                                "JUDGE",
                                Firstname .. " " .. Lastname .. " Is now in jail for the reason: " .. args[3],
                            },
                            color = { 249, 166, 0 },
                        })
                    end)
                ...
                TriggerClientEvent("esx:showNotification", src, TranslateCap("invalid_time"))
            end
        else
            TriggerClientEvent("esx:showNotification", src, TranslateCap("id_not_online"))
        end
    else
        TriggerClientEvent("esx:showNotification", src, TranslateCap("not_officer"))
    end
end)
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
    ...
    if xPlayer.job.name == "police" then
        ...
        GetRPName(targetSrc, function(Firstname, Lastname)
            TriggerClientEvent("chat:addMessage", -1, {
                args = { "JUDGE", Firstname .. " " .. Lastname .. " Is now in jail for the reason: " .. jailReason },
                color = { 249, 166, 0 },
            })
        end)
    else
...
AddEventHandler("esx-qalle-jail:prisonWorkReward", function()
    ...
    TriggerClientEvent("esx:showNotification", src, TranslateCap("money_for_food"))
end)
