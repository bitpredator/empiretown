...
function setLight(ant, cat, item, state) {
	...
	const obj = elements.antennas[ant][cat][item];
...
function clearDirs(ant) {
	...
	for (const i in elements.antennas[ant].dirs) {
...
function clearEverything() {
	...
	for (const i in elements.antennas) {
		clearAntenna(i);
	}
}
...
function playAudio(name, vol) {
	...
	const audio = new Audio('sounds/' + audioNames[name]);
...
function displayKeyLock(state) {
	const sl = elements.keyLock.stateLabel;
...
function loadUiSettings(data, isSave) {
	...
	for (const setting of ['remote', 'radar', 'plateReader']) {
		const ele = elements[setting];

		if (isSave) {
			// Iterate through the settings
			for (const i of ['left', 'top']) {
			...
			const w = (ele.outerWidth() * data.scale[setting]);
			const h = (ele.outerHeight() * data.scale[setting]);
...
elements.setBoloBtn.click(function() {
	...
	const plate = elements.boloText.val().toUpperCase();

	// Gets the amount of whitespace there should be
	const spaceAmount = 8 - plate.length;

	if (spaceAmount > 0) {
		// Splits the amount in half
		const split = spaceAmount / 2;
		...
		const startSpace = Math.floor(split);
		const endSpace = Math.ceil(split);
...
function checkPlateInput(event) {
	...
	const valid = /[a-zA-Z0-9 ]/g.test(event.key);
...
let remoteOffset = [0, 0];

// eslint-disable-next-line no-var
var radarScale = 1.0;
let radarMoving = false;
let radarOffset = [0, 0];

// eslint-disable-next-line no-var
var readerScale = 1.0;
let readerMoving = false;
let readerOffset = [0, 0];
...
function calculatePos(ele, x, y, w, h, offset, scale, safezone) {
	const eleWidth = (ele.outerWidth() * scale);
	const eleHeight = (ele.outerHeight() * scale);
	const eleWidthPerct = (eleWidth / w) * 100;
	const eleHeightPerct = (eleHeight / h) * 100;
	const eleWidthPerctHalf = eleWidthPerct / 2;
	const eleHeightPerctHalf = eleHeightPerct / 2;

	const maxWidth = w - eleWidth;
	const maxHeight = h - eleHeight;

	const left = clamp(x + offset[0], 0 + safezone, maxWidth - safezone);
	const top = clamp(y + offset[1], 0 + safezone, maxHeight - safezone);

	let leftPos = (left / w) * 100;
	let topPos = (top / h) * 100;

	const leftLockGap = leftPos + eleWidthPerctHalf;
	const topLockGap = topPos + eleHeightPerctHalf;
...
function changeEleScale(ele, scaleVar, amount, display) {
	...
	const scale = changeScale(ele, scaleVar, amount);
...
function changeScale(ele, current, amount) {
	const scale = clamp(current + amount, 0.25, 2.5);
...
$('body').find('button, div').each(function(i, obj) {
	if ($(this).attr('data-nuitype')) {
		$(this).click(function() {
			const type = $(this).data('nuitype');
			const value = $(this).attr('data-value') ? $(this).data('value') : null;
			const mode = $(this).attr('data-mode') ? $(this).data('mode') : null;
