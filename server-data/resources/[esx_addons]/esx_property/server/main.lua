local function Log(title, color, fields, Level)
	if Level <= Config.Logs.LogLevel then
		...
		PerformHttpRequest(
			...
			function() end,
			"POST",
			json.encode({ username = "ESX-Framework", embeds = embedData }),
...
function PropertiesRefresh()
	...
	if PropertiesList then
		...
		Log(
			...
			11141375,
			{
				{ name = "Property Count", value = #Properties, inline = true },
				{ name = "OX Inventory", value = Config.OxInventory and "Enabled" or "Disabled", inline = true },
			},
			1
		)
...
function IsPlayerAdmin(player, action)
	...
	if Config.PlayerManagement.Enabled and action then
		if
			xPlayer.job.name == Config.PlayerManagement.job
			and xPlayer.job.grade >= Config.PlayerManagement.Permissions[action]
		then
			return true
		end
	end
...
CreateThread(function()
	...
	MySQL.insert(
		"INSERT IGNORE INTO `datastore` (name, label, shared) VALUES ('property', 'Property' , 1)",
		function(result)
			if result then
	...
	MySQL.insert(
		"INSERT IGNORE INTO `datastore_data` (name, owner, data) VALUES ('property', NULL, '{}')",
		function(result)
			if result then
	...
	if PM.Enabled then
		MySQL.insert(
			"INSERT IGNORE INTO `addon_account` (name, label, shared) VALUES (?, ? , 1)",
			{ PM.society, PM.joblabel },
			function(result)
				if result then
					print(
						"[^2INFO^7] Adding ^5"
		...
		for i = 1, #PM.jobRanks do
			...
			if not Existance then
				MySQL.query(
					...
					{ PM.job, PM.jobRanks[i].grade, PM.jobRanks[i].name, PM.jobRanks[i].label, PM.jobRanks[i].salary },
					function(result)
						if result then
							print(
								"[^2INFO^7] Inserting ^5"
...
ESX.RegisterServerCallback("esx_property:buyFurniture",
	function(source, cb, PropertyId, PropName, PropIndex, PropCatagory, pos, heading)
		...
		if
			xPlayer.identifier == Owner
			or IsPlayerAdmin(source)
			or (Properties[PropertyId].Keys and Properties[PropertyId].Keys[xPlayer.identifier])
		then
			...
			if xPlayer.getAccount("bank").money >= Price then
				...
				for i = 1, #Properties[PropertyId].plysinside do
					TriggerClientEvent(
						"esx_property:placeFurniture",
						Properties[PropertyId].plysinside[i],
						PropertyId,
						furniture,
		...
		Log("Furniture Bought", 3640511, {
			...
			{
				name = "**Prop Name**",
				value = Config.FurnitureCatagories[PropCatagory][PropIndex].title,
				inline = true,
			},
			{
				name = "**Price**",
				value = tostring(Config.FurnitureCatagories[PropCatagory][PropIndex].price),
				inline = true,
			},
			{
				name = "**Can Afford**",
				value = xPlayer.getAccount("bank").money >= Config.FurnitureCatagories[PropCatagory][PropIndex].price
						and "Yes"
					or "No",
				inline = true,
			},
	...
	if xPlayer.identifier == Owner then
		...
		Log("Property Sold", 16711680, {
			...
			{ name = "**Furniture Count**", value = tostring(furn), inline = true },
			{
				name = "**Vehicle Count**",
				value = tostring(
					Properties[PropertyId].garage.StoredVehicles and #Properties[PropertyId].garage.StoredVehicles
						or "N/A"
				),
				inline = true,
			},
...
ESX.RegisterServerCallback("esx_property:SetGaragePos", function(source, cb, PropertyId, heading)
	...
	if IsPlayerAdmin(source, "ToggleGarage") then
		...
		local Original = Properties[PropertyId].garage.pos
				and Properties[PropertyId].garage.pos.x .. ", " .. Properties[PropertyId].garage.pos.y .. ", " .. Properties[PropertyId].garage.pos.z
			or "N/A"
...
ESX.RegisterServerCallback("esx_property:CCTV", function(source, cb, PropertyId)
	...
	then
		SetEntityCoords(
			GetPlayerPed(source),
			vector3(Property.Entrance.x, Property.Entrance.y, Property.Entrance.z + 5.0)
		)
...
ESX.RegisterServerCallback("esx_property:SetPropertyName", function(source, cb, PropertyId, name)
	...
	cb(
		(xPlayer.identifier == Owner or IsPlayerAdmin(source, "SetPropertyName"))
			and name
			and #name <= Config.MaxNameLength
	)
end)
...
ESX.RegisterServerCallback("esx_property:deleteProperty", function(source, cb, PropertyId)
	...
	if IsPlayerAdmin(source, "DeleteProperty") then
		Log("Property Deleted", 16711680, {
			...
			{
				name = "**Owner**",
				value = Properties[PropertyId].OwnerName ~= "" and Properties[PropertyId].OwnerName or "N/A",
				inline = true,
			},
			{ name = "**Furniture Count**", value = #Properties[PropertyId].furniture, inline = true },
			{
				name = "**Vehicle Count**",
				value = Properties[PropertyId].garage.StoredVehicles and #Properties[PropertyId].garage.StoredVehicles
					or "N/A",
				inline = true,
			},
		}, 1)
...
ESX.RegisterServerCallback("esx_property:ChangeInterior", function(source, cb, PropertyId, Interior)
	...
	if IsPlayerAdmin(source, "ChangeInterior") then
		...
		Log("Property Interior Changed", 3640511, {
			...
			{
				name = "**New Interior**",
				value = GetInteriorValues(Properties[PropertyId].Interior).label,
				inline = true,
			},
		}, 2)
	end
...
ESX.RegisterServerCallback("esx_property:RemoveAllfurniture", function(source, cb, PropertyId)
	...
	if
		xPlayer.identifier == Owner
		or IsPlayerAdmin(source, "ResetFurniture")
		or (Properties[PropertyId].Keys and Properties[PropertyId].Keys[xPlayer.identifier])
	then
		for i = 1, #Properties[PropertyId].plysinside do
			for furniture = 1, #Properties[PropertyId].furniture do
				TriggerClientEvent(
					"esx_property:removeFurniture",
					Properties[PropertyId].plysinside[i],
					PropertyId,
					furniture
				)
...
ESX.RegisterServerCallback("esx_property:deleteFurniture", function(source, cb, PropertyId, furnitureIndex)
	...
	Log(
		"Property Furniture Deleted",
		16711680,
		{
			{ name = "**Property Name**", value = Properties[PropertyId].Name, inline = true },
			{ name = "**Admin**", value = xPlayer.getName(), inline = true },
			{ name = "**Furniture Name**", value = xPlayer.getName(), inline = true },
		},
		3
	)
	cb(
				...
				if itemCount >= Config.Raiding.ItemRequired.ItemCount then
					if Config.Raiding.ItemRequired.RemoveItem then
						xPlayer.removeInventoryItem(
							Config.Raiding.ItemRequired.name,
							Config.Raiding.ItemRequired.ItemCount
						)
					end
					Can = true
				else
					xPlayer.showNotification(
						_U("raid_notify_error", Config.Raiding.ItemRequired.ItemCount, Config.Raiding.ItemRequired.name),
						"error"
					)
				end
			else
...
ESX.RegisterServerCallback("esx_property:ChangeEntrance", function(source, cb, PropertyId, Coords)
	...
	if IsPlayerAdmin(source, "ChangeEntrance") then
		local Origonal = Properties[PropertyId].Entrance.x
			.. ","
			.. Properties[PropertyId].Entrance.y
			.. ","
			.. Properties[PropertyId].Entrance.z
		Properties[PropertyId].Entrance =
			{ x = ESX.Math.Round(Coords.x, 2), y = ESX.Math.Round(Coords.y, 2), z = ESX.Math.Round(Coords.z, 2) - 0.8 }
		TriggerClientEvent("esx_property:syncProperties", -1, Properties)
		Log("Property Entrance Changed", 3640511, {
			...
			{
				name = "**New**",
				value = Properties[PropertyId].Entrance.x
					.. ","
					.. Properties[PropertyId].Entrance.y
					.. ","
					.. Properties[PropertyId].Entrance.z,
...
ESX.RegisterServerCallback("esx_property:SetInventoryPosition", function(source, cb, PropertyId, Coords, Reset)
	if Config.OxInventory then
		...
		local xPlayer = ESX.GetPlayerFromId(source)
		if
			IsPlayerAdmin(source, "EditInteriorPositions")
			or (Property.Owner == xPlayer.identifier or Properties[PropertyId].Keys[xPlayer.identifier])
		then
			local Interior = GetInteriorValues(Property.Interior)
			if Reset then
				...
				if Interior.type == "ipl" then
					...
					Properties[PropertyId].positions.Storage = vector3(Property.Entrance.x, Property.Entrance.y, 2000)
						- Coords
				end
			end
	...
	then
		...
		if Reset then
			...
			if Interior.type == "ipl" then
				...
				Properties[PropertyId].positions.Wardrobe = vector3(Property.Entrance.x, Property.Entrance.y, 1999.8)
					- Coords
			end
...
ESX.RegisterServerCallback("esx_property:GetNearbyPlayers", function(source, cb, property)
	...
	local xPlayer = ESX.GetPlayerFromId(source)
	local NearbyPlayers =
		ESX.OneSync.GetPlayersInArea(vector3(Property.Entrance.x, Property.Entrance.y, Property.Entrance.z), 5.0)
	Wait(100)
	for _, v in pairs(NearbyPlayers) do
...
ESX.RegisterServerCallback("esx_property:StoreVehicle", function(source, cb, PropertyId, VehicleProperties)
						...
						if result then
							if result == xPlayer.identifier then
								Properties[PropertyId].garage.StoredVehicles[#Properties[PropertyId].garage.StoredVehicles + 1] =
									{ owner = xPlayer.identifier, vehicle = VehicleProperties }
								cb(true)
	...
	Log("User Attempted To Store Vehicle", 3640511, {
		...
		{
			name = "**Has Access**",
			value = (Property.Owner == xPlayer.identifier or Properties[PropertyId].Keys[xPlayer.identifier]) and "Yes"
				or "No",
			inline = true,
		},
		...
		if Property.Keys then
			if Properties[property].Keys[player] then
				Log("Property Key Revoked", 3640511, {
					...
					{
						name = "**Removed From**",
						value = tostring(Properties[property].Keys[player].name),
						inline = true,
					},
				}, 3)
...
RegisterNetEvent("esx_property:enter", function(PropertyId)
	...
	MySQL.query(
		"UPDATE `users` SET `last_property` = ? WHERE `identifier` = ?",
		{ json.encode(PropertyData), xPlayer.identifier }
	...
	Log(
		...
		3640511,
		{
			{ name = "**Property Name**", value = Properties[PropertyId].Name, inline = true },
			{ name = "**Player**", value = xPlayer.getName(), inline = true },
			{ name = "**Property Player Count**", value = tostring(#Properties[PropertyId].plysinside), inline = true },
		},
		3
	)
end)
...
ESX.RegisterServerCallback("esx_property:CanAccessRealEstateMenu", function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	local Re = (
		Config.PlayerManagement.Enabled
		and xPlayer.job.name == Config.PlayerManagement.job
		and xPlayer.job.grade >= Config.PlayerManagement.Permissions.ManagePropertiesFromQuickActions
	)
			and true
		or false
	cb(Re)
...
RegisterNetEvent("esx_property:server:createProperty", function(Property)
	...
	local xPlayer = ESX.GetPlayerFromId(source)
	local Interior = GetInteriorValues(Property.interior)
	local garageData = Property.garage.enabled
			and { enabled = true, pos = Property.garage.pos, Heading = Property.garage.heading, StoredVehicles = {} }
		or { enabled = false }
	if IsPlayerAdmin(source, "CreateProperty") then
	...
	Log("Property Created", 65280, {
		...
		{
			name = "**Entrance**",
			value = tostring(Property.entrance.x .. ", " .. Property.entrance.y .. ", " .. Property.entrance.z),
			inline = true,
		},
	}, 1)
	TriggerClientEvent("esx_property:syncProperties", -1, Properties)
		...
		CreateThread(function()
			...
			if Properties and #Properties > 0 then
				...
				Log(
					...
					11141375,
					{
						{ name = "**Reason**", value = "Scheduled Server Restart", inline = true },
						{ name = "**Property Count**", value = tostring(#Properties), inline = true },
					},
					1
				)
			end
