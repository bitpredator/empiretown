...
function GiveKeysMenu(Property)
    ESX.TriggerServerCallback("esx_property:GetInsidePlayers", function(Players)
        local Elements = { { unselectable = true, title = TranslateCap("nearby"), icon = "fas fa-user-plus" }, { title = TranslateCap("back"), icon = "fas fa-arrow-left", value = "go_back" } }
...
function RemoveKeysMenu(Property)
    ESX.TriggerServerCallback("esx_property:GetPlayersWithKeys", function(Players)
        local Elements = { { unselectable = true, title = TranslateCap("remove_title"), icon = "fas fa-user-plus" }, { title = TranslateCap("back"), icon = "fas fa-arrow-left", value = "go_back" } }
...
function ManageKeys(Property)
    ESX.HideUI()
    local Elements = {
        { unselectable = true, title = TranslateCap("key_management"), icon = "fas fa-key" },
...
function SetPropertyName(Property)
    ...
    local Elements = {
        { unselectable = true, title = TranslateCap("name_edit"), icon = "fa-solid fa-signature" },
        { icon = "", title = TranslateCap("name"), input = true, inputType = "text", inputValue = Name, inputPlaceholder = Properties[Property].Name, name = "setName" },
        { icon = "fa-solid fa-check", title = TranslateCap("confirm"), name = "confirm" },
    }
...
function PropertyMenuElements(PropertyId)
    ...
    if Property.Owned then
        ...
        if ESX.PlayerData.identifier == Property.Owner then
            table.insert(elements, { title = TranslateCap("name_manage"), description = TranslateCap("name_manage_desc"), icon = "fa-solid fa-signature", value = "property_name" })
        end
        if not InProperty then
            if ESX.PlayerData.identifier == Property.Owner then
                ...
                table.insert(elements, { title = TranslateCap("sell_title"), description = TranslateCap("sell_desc", ESX.Math.GroupDigits(ESX.Round(Property.Price * 0.6))), icon = "fas fa-dollar-sign", value = "property_sell" })
            end
            ...
            if ESX.PlayerData.identifier == Property.Owner or PlayerKeys[PropertyId] then
                if Config.CanCustomiseInventoryAndWardrobePositions then
                    if Config.OxInventory then
                        table.insert(elements, { title = TranslateCap("inventory_title"), description = TranslateCap("inventory_desc"), icon = "fa-solid fa-up-down-left-right", value = "property_inventory" })
                    end
                    table.insert(elements, { title = TranslateCap("wardrobe_title"), description = TranslateCap("wardrobe_desc"), icon = "fa-solid fa-up-down-left-right", value = "property_wardrobe" })
                end
                table.insert(elements, { title = TranslateCap("furniture_title"), description = TranslateCap("furniture_desc"), icon = "fas fa-boxes-stacked", value = "property_furniture" })
            end
        end
        ...
        if not PM.Enabled then
            table.insert(elements, { title = TranslateCap("buy_title"), description = TranslateCap("buy_desc", ESX.Math.GroupDigits(ESX.Round(Property.Price))), icon = "fas fa-shopping-cart", value = "property_buy" })
        else
            if ESX.PlayerData.job.name == PM.job and ESX.PlayerData.job.grade >= PM.Permissions.SellProperty then
                table.insert(elements, { title = TranslateCap("sellplayer_title"), description = TranslateCap("sellplayer_desc", ESX.Math.GroupDigits(ESX.Round(Property.Price))), icon = "fas fa-shopping-cart", value = "property_sell_re" })
            end
        end
...
RegisterNetEvent("esx_property:CreateProperty", function()
    ESX.TriggerServerCallback("esx_property:CanCreateProperty", function(data)
        if data then
            ...
            HouseData = {}
            local Property = {
                { unselectable = true, icon = "fas fa-plus", title = TranslateCap("menu_title") },
                {
                    value = 0,
                    title = TranslateCap("element_title1"),
                    icon = "fas fa-list-ol",
                    description = TranslateCap("element_description1"),
                    input = true,
                    inputType = "number",
                ...
                {
                    title = TranslateCap("element_title2"),
                    icon = "fas fa-dollar-sign",
                    input = true,
                    inputType = "number",
                    description = TranslateCap("element_description2"),
                ...
                {
                    title = TranslateCap("element_title5"),
                    description = TranslateCap("element_description5"),
                    icon = "fas fa-video",
                ...
                ESX.OpenContext("right", Property, function(menu, element)
                    ...
                    if element.index then
                        ...
                        if element.index == "interior" then
                            local catsa = { { unselectable = true, icon = "fas fa-warehouse", title = TranslateCap("types_title") }, { title = TranslateCap("ipl_title"), description = TranslateCap("ipl_description"), index = "IPL" } }
                        ...
                        if element.index == "cctv" then
                            ...
                            opos = {
                                { unselectable = true, icon = "fas fa-video", title = TranslateCap("cctv_settings") },
                                { title = TranslateCap("toggle_title"), icon = status == TranslateCap("enabled") and "fas fa-eye" or "fas fa-eye-slash", description = TranslateCap("toggle_description", status), index = "ToggleCCTV" },
                                { title = TranslateCap("cctv_set_title"), icon = "fas fa-rotate", disabled = not Property[6].value.enabled, description = TranslateCap("cctv_set_description"), index = "SetCCTVangle" },
                        ...
                        if Config.Garage.Enabled and element.index == "garage" then
                            ...
                            opos = {
                                { unselectable = true, icon = "fas fa-warehouse", title = TranslateCap("garage_settings") },
                                { title = TranslateCap("toggle_title"), icon = status == TranslateCap("enabled") and "fa-solid fa-toggle-on" or "fa-solid fa-toggle-off", description = TranslateCap("toggle_description", status), index = "ToggleGarage" },
                            }
                            if Property[5].value.enabled then
                                opos[#opos + 1] = { title = TranslateCap("garage_set_title"), icon = "fas fa-map-marker-alt", disabled = not Property[5].value.enabled, description = TranslateCap("garage_set_description"), index = "SetGaragePos" }
                        ...
                        ESX.OpenContext("right", GetData(), function(menu, element)
                            if element.value then
                                ...
                                if element.value == "garage" then
                                    ...
                                    opos = {
                                        { unselectable = true, icon = "fas fa-warehouse", title = TranslateCap("garage_settings") },
                                        { title = "Toggle Usage", icon = status == TranslateCap("enabled") and "fa-solid fa-toggle-on" or "fa-solid fa-toggle-off", description = "Current Status: " .. status, value = "ToggleGarage" },
                                    }
                                ...
                                if element.value == "cctv" then
                                    ...
                                    opos = {
                                        { unselectable = true, icon = "fas fa-warehouse", title = TranslateCap("cctv_settings") },
                                        { title = "Toggle Usage", icon = status == TranslateCap("enabled") and "fa-solid fa-toggle-on" or "fa-solid fa-toggle-off", description = "Current Status: " .. status, value = "ToggleCCTV" },
                                    }
