...
RegisterNetEvent("esx_property:syncProperties", function(properties, lastProperty)
	...
	if lastProperty then
		if
			Properties[lastProperty.id]
			and (Properties[lastProperty.id].Owner == ESX.PlayerData.identifier or PlayerKeys[lastProperty.id])
		then
			...
			ESX.TriggerServerCallback("esx_property:RemoveLastProperty", function()
				SetEntityCoords(
					ESX.PlayerData.ped,
					vector3(lastProperty.coords.x, lastProperty.coords.y, lastProperty.coords.z)
				)
			end)
		end
	end
...
function GiveKeysMenu(Property)
	ESX.TriggerServerCallback("esx_property:GetInsidePlayers", function(Players)
		local Elements = {
			{ unselectable = true, title = _U("nearby"), icon = "fas fa-user-plus" },
			{ title = _U("back"), icon = "fas fa-arrow-left", value = "go_back" },
		}
		for i = 1, #Players, 1 do
			Elements[#Elements + 1] =
				{ title = Players[i].Name, icon = "far fa-user", index = Players[i].Id, value = "user" }
		end
...
function RemoveKeysMenu(Property)
	ESX.TriggerServerCallback("esx_property:GetPlayersWithKeys", function(Players)
		local Elements = {
			{ unselectable = true, title = _U("remove_title"), icon = "fas fa-user-plus" },
			{ title = _U("back"), icon = "fas fa-arrow-left", value = "go_back" },
		}
...
function SetPropertyName(Property)
	...
	local Elements = {
		{ unselectable = true, title = _U("name_edit"), icon = "fa-solid fa-signature" },
		{
			...
			inputType = "text",
			inputValue = Name,
			inputPlaceholder = Properties[Property].Name,
			name = "setName",
		},
...
function PropertyMenuElements(PropertyId)
	...
	local elements = {
		{
			unselectable = true,
			title = Property.setName ~= "" and Property.setName or Property.Name,
			icon = "fas fa-home",
		},
	}
	if Property.Owned then
		...
		if ESX.PlayerData.identifier == Property.Owner then
			table.insert(
				elements,
				{
					title = _U("name_manage"),
					description = _U("name_manage_desc"),
		...
		if not InProperty then
			if ESX.PlayerData.identifier == Property.Owner then
				table.insert(
					elements,
					{
						title = _U("key_management"),
						description = _U("key_management_desc"),
						icon = "fas fa-key",
						value = "property_keys",
					}
				)
				table.insert(
					...
					{
						title = _U("sell_title"),
						description = _U("sell_desc", ESX.Math.GroupDigits(ESX.Round(Property.Price * 0.6))),
			...
			if
				Config.Raiding.Enabled
				and Property.Locked
				and ESX.PlayerData.job
				and ESX.GetPlayerData().job.name == "police"
			then
				table.insert(
					elements,
					{
						title = _U("raid_title"),
						description = _U("raid_desc"),
						icon = "fas fa-bomb",
						value = "property_raid",
					}
				)
			...
			if
				(Config.CCTV.Enabled and Properties[PropertyId].cctv.enabled)
				and (ESX.PlayerData.identifier == Property.Owner or PlayerKeys[PropertyId])
			then
				table.insert(
					elements,
					{
						title = _U("cctv_title"),
						description = _U("cctv_desc"),
						icon = "fas fa-video",
						value = "property_cctv",
					}
				)
			...
			if ESX.PlayerData.identifier == Property.Owner or PlayerKeys[PropertyId] then
				if Config.CanCustomiseInventoryAndWardrobePositions then
					if Config.OxInventory then
						table.insert(
							elements,
							{
								title = _U("inventory_title"),
								description = _U("inventory_desc"),
					...
					table.insert(
						elements,
						{
							title = _U("wardrobe_title"),
							description = _U("wardrobe_desc"),
							icon = "fa-solid fa-up-down-left-right",
				...
				table.insert(
					elements,
					{
						title = _U("furniture_title"),
						description = _U("furniture_desc"),
		...
		if
			(not Property.Locked or Config.OwnerCanAlwaysEnter and ESX.PlayerData.identifier == Property.Owner)
			and not InProperty
		then
		...
		if
			...
			and not InProperty
			and not (
				Config.OwnerCanAlwaysEnter and ESX.PlayerData.identifier == Property.Owner or PlayerKeys[PropertyId]
			)
		then
			table.insert(
				elements,
				{ title = _U("knock_title"), icon = "fa-solid fa-hand-sparkles", value = "property_knock" }
			)
		end
	else
		if not PM.Enabled then
			table.insert(
				...
				{
					title = _U("buy_title"),
					description = _U("buy_desc", ESX.Math.GroupDigits(ESX.Round(Property.Price))),
			...
			if ESX.PlayerData.job.name == PM.job and ESX.PlayerData.job.grade >= PM.Permissions.SellProperty then
				table.insert(
					elements,
					{
						title = _U("sellplayer_title"),
						description = _U("sellplayer_desc", ESX.Math.GroupDigits(ESX.Round(Property.Price))),
						icon = "fas fa-shopping-cart",
		...
		while InProperty do
			...
			if Interior.type == "shell" then
				if #(PlayerCoords - vector3(Property.Entrance.x, Property.Entrance.y, 1999)) < 5.0 then
					...
					DrawMarker(
						27,
						vector3(Property.Entrance.x, Property.Entrance.y, 2000.2),
						0.0,
						0.0,
						0.0,
						0.0,
					...
					if #(PlayerCoords.xy - vector2(Property.Entrance.x, Property.Entrance.y)) <= 2.5 then
						...
						if not ShowingUIs.Exit then
							local Pname = Properties[CurrentId].setName ~= "" and Properties[CurrentId].setName
								or Properties[CurrentId].Name
				...
				if #(PlayerCoords - Interior.pos) < 5.0 then
					...
					DrawMarker(
						27,
						vector3(Interior.pos.xy, Interior.pos.z - 0.98),
						0.0,
						0.0,
						0.0,
						0.0,
	...
	ESX.TriggerServerCallback("esx_property:AccessGarage", function(Vehicles)
		if Vehicles then
			...
			for k, v in pairs(Vehicles) do
				elements[#elements + 1] =
					{ title = v.vehicle.DisplayName .. " | " .. v.vehicle.plate, Properties = v.vehicle, index = k }
			end
			ESX.OpenContext("right", elements, function(menu, element)
					...
					if
						ESX.Game.IsSpawnPointClear(
							vector3(
								...
								Properties[PropertyId].garage.pos.y,
								Properties[PropertyId].garage.pos.z
							),
							3.0
						)
					then
						ESX.Game.SpawnVehicle(
							...
							Properties[PropertyId].garage.pos,
		...
		if #Properties > 0 then
			...
			for i = 1, #Properties do
				if Properties[i].Entrance then
					...
					local Entrance =
						vector3(Properties[i].Entrance.x, Properties[i].Entrance.y, Properties[i].Entrance.z)
					if not InCCTV then
						if #(Coords - Entrance) < 10.0 then
							...
							if #(Coords - Entrance) < 1.5 then
								nearby = true
								local PropertyName = Properties[i].setName ~= "" and Properties[i].setName
									or Properties[i].Name
					...
					if
						Properties[i].garage.enabled
						and Properties[i].garage.pos
						and (ESX.PlayerData.identifier == Properties[i].Owner or PlayerKeys[i])
					then
						local Garage =
							vector3(Properties[i].garage.pos.x, Properties[i].garage.pos.y, Properties[i].garage.pos.z)
...
RegisterNetEvent("esx_property:CreateProperty", function()
	ESX.TriggerServerCallback("esx_property:CanCreateProperty", function(data)
		if data then
			...
			local SuggestedPrice = Config.ZonePriceOptions.Enabled
					and Config.ZonePriceOptions.Default * Config.ZonePrices[ZoneScum]
				or nil
			HouseData = {}
			local Property = {
				...
				{
					value = 0,
					title = _U("element_title1"),
					icon = "fas fa-list-ol",
					description = _U("element_description1"),
					input = true,
					inputType = "number",
				...
				{
					title = _U("element_title2"),
					icon = "fas fa-dollar-sign",
					input = true,
					inputType = "number",
					description = _U("element_description2"),
				...
				{
					title = _U("element_title3"),
					description = _U("element_description3"),
					icon = "fas fa-home",
					index = "interior",
				},
				{
					title = _U("element_title4"),
					description = _U("element_description4"),
					icon = "fas fa-warehouse",
					value = { enabled = false },
					index = "garage",
					disabled = not Config.Garage.Enabled,
				},
				{
				...
				{
					title = _U("element_title6"),
					description = _U("element_description6"),
					icon = "fas fa-map-marker-alt",
					index = "entrance",
				},
				{
					title = _U("element_create_title"),
					icon = "fas fa-check",
					description = _U("element_create_desc_1"),
					index = "creation",
				},
			}
					...
					if element.index then
						if element.index == "entrance" then
							...
							HouseData.entrance = {
								...
								z = ESX.Math.Round(PlayerPos.z, 2) - 0.98,
							}
							Property[7].title = _U("entrance_set_title")
							Property[7].description = _U(
								"entrance_set_description",
								HouseData.entrance.x,
								HouseData.entrance.y,
								HouseData.entrance.z
							)
							OpenCreate()
						end
						...
						if element.index == "IPL" then
							...
							for i = 1, #Config.Interiors.IPL do
								ints[#ints + 1] = {
									...
									value = Config.Interiors.IPL[i].value,
								}
						...
						if element.index == "Shells" then
							local ints =
								{ { unselectable = true, icon = "fas fa-warehouse", title = _U("shell_title") } }
							for i = 1, #Config.Interiors.Shells do
								ints[#ints + 1] = {
									title = Config.Interiors.Shells[i].label,
									index = "selectedinterior",
									value = Config.Interiors.Shells[i].value,
								}
								exports["esx_context"]:Refresh(ints, "right")
							end
						end
						if element.index == "interior" then
							local catsa = {
								{ unselectable = true, icon = "fas fa-warehouse", title = _U("types_title") },
								{ title = _U("ipl_title"), description = _U("ipl_description"), index = "IPL" },
							}
							if Config.Shells then
								catsa[3] = {
									title = _U("shell_title"),
									description = _U("shell_description"),
									index = "Shells",
								}
							end
						...
						if element.index == "cctv" then
							...
							opos = {
								...
								{
									title = _U("toggle_title"),
									icon = status == _U("enabled") and "fas fa-eye" or "fas fa-eye-slash",
									description = _U("toggle_description", status),
									index = "ToggleCCTV",
								},
								{
									title = _U("cctv_set_title"),
									icon = "fas fa-rotate",
									disabled = not Property[6].value.enabled,
									description = _U("cctv_set_description"),
									index = "SetCCTVangle",
								},
								{
									title = _U("back"),
									icon = "fas fa-arrow-left",
									description = _U("back_description"),
									index = "return",
								},
							}
						...
						if Config.Garage.Enabled and element.index == "garage" then
							...
							opos = {
								...
								{
									title = _U("toggle_title"),
									icon = status == _U("enabled") and "fa-solid fa-toggle-on"
										or "fa-solid fa-toggle-off",
									description = _U("toggle_description", status),
									index = "ToggleGarage",
								},
							}
							if Property[5].value.enabled then
								opos[#opos + 1] = {
									title = _U("garage_set_title"),
									icon = "fas fa-map-marker-alt",
									disabled = not Property[5].value.enabled,
									description = _U("garage_set_description"),
									index = "SetGaragePos",
								}
		...
		if data then
					...
					if data then
						...
						local function GetData()
							local elements = {
								...
								{
									title = "Enter",
									description = "Force Entry Into The Property.",
								...
								},
								{
									title = "Price",
									icon = "fas fa-dollar-sign",
									description = "Alter The Price Of The Property.",
									value = "price",
								},
								{
									title = "Set Interior",
									description = "Renovate The Property`s Interior.",
									icon = "fas fa-home",
									value = "interior",
								},
								{
							...
							if
								...
								and (
									ESX.Round(Interior.positions.Storage.x, 2)
										~= Properties[currentProperty].positions.Storage.x
									or ESX.Round(Interior.positions.Storage.y, 2)
										~= Properties[currentProperty].positions.Storage.y
								)
							then
								elements[#elements + 1] = {
							...
							if Interior.positions.Wardrobe then
								if
									ESX.Round(Interior.positions.Wardrobe.x, 2)
										~= ESX.Round(Properties[currentProperty].positions.Wardrobe.x, 2)
									or ESX.Round(Interior.positions.Wardrobe.y, 2)
										~= ESX.Round(Properties[currentProperty].positions.Wardrobe.y, 2)
								then
									elements[#elements + 1] = {
						...
						ESX.OpenContext("right", GetData(), function(menu, element)
							if element.value then
								...
								if element.value == "garage" then
									local status = Properties[currentProperty].garage.enabled and _U("enabled")
										or _U("disabled")
									opos = {
										{
											unselectable = true,
											icon = "fas fa-warehouse",
											title = _U("garage_settings"),
										},
										{
											title = "Toggle Usage",
											icon = status == _U("enabled") and "fa-solid fa-toggle-on"
								...
								if element.value == "cctv" then
									local status = Properties[currentProperty].cctv.enabled and _U("enabled")
										or _U("disabled")
									opos = {
											...
											ESX.TriggerServerCallback("esx_property:SetGaragePos", function(IsChanged)
												if IsChanged then
													...
													ESX.ShowNotification(
														"You ~r~Cannot~s~ Change This Option!",
														"error"
													)
												end
								...
								if element.value == "interior" then
									local elements = {
										{ unselectable = true, icon = "fas fa-warehouse", title = "Interior Types" },
										{
											title = "IPL Interiors",
											description = "Native GTA Interiors, Made by R*",
											value = "IPL",
										},
									}
									if Config.Shells then
									...
									ESX.OpenContext("right", elements, function(menu, element)
										if element.value then
											local elements = {
												{ unselectable = true, icon = "fas fa-warehouse", title = "Interiors" },
											}
											for i = 1, #Config.Interiors[element.value] do
												elements[#elements + 1] = {
													title = Config.Interiors[element.value][i].label,
													value = Config.Interiors[element.value][i].value,
												}
											end
											ESX.OpenContext("right", elements, function(menu, element)
												if element.value then
													ESX.TriggerServerCallback(
														"esx_property:ChangeInterior",
														function(IsChanged)
															if IsChanged then
					...
					if data then
						local elements = {
							...
							{ title = _U("back"), icon = "fas fa-arrow-left", value = "back" },
							{
								title = "Manage",
								icon = "fas fa-cogs",
								description = "Alter This Property's Settings.",
								value = "manage",
							},
							{
								title = "Teleport",
								description = "Teleport To This Property.",
								icon = "fas fa-map-marker-alt",
								value = "goto",
							},
							{
								title = "Set GPS",
								description = "Set GPS position To Property.",
							...
							},
							{
								...
								description = "Remove Current Property.",
								value = "delete",
							},
						}
						...
						ESX.OpenContext("right", elements, function(menu, element)
							if element.value then
								if element.value == "goto" then
									SetEntityCoords(
										ESX.PlayerData.ped,
										Properties[currentProperty].Entrance.x,
			...
			function AdminMenu()
				ESX.TriggerServerCallback("esx_property:IsAdmin", function(data)
					if data then
						local elements =
							{ { unselectable = true, icon = "fas fa-home", title = "Properties Management" } }
						for i = 1, #Properties do
							if Properties[i].Entrance then
								local description = ""
