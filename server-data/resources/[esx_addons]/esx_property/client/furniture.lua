if Config.Furniture.Enabled then
	...
	RegisterNetEvent("esx_property:placeFurniture", function(Id, furniture, findex)
		if Properties[Id] and InProperty and CurrentId == Id then
			ESX.Game.SpawnLocalObject(
				furniture.Name,
				vector3(furniture.Pos.x, furniture.Pos.y, furniture.Pos.z - 0.1),
				function(object)
			...
			while true do
				...
				if IsControlPressed(0, Config.Furniture.Controls.MinusY) then
					...
					CurrentPos = CurrentPos
						- vector3(
							direction.x * Config.Furniture.MovementSpeed,
							direction.y * Config.Furniture.MovementSpeed,
							0
						)
						...
						ESX.OpenContext("right", {
							{
								...
								title = TranslateCap(
									"confirm_buy",
									Config.FurnitureCatagories[PropCatagory][PropIndex].title
								),
								description = TranslateCap(
									"price",
									Config.FurnitureCatagories[PropCatagory][PropIndex].price
								),
							...
							if element.value and element.value == "buy" then
								ESX.TriggerServerCallback(
									...
									PropIndex,
									PropCatagory,
									vector3(CurrentPos.x, CurrentPos.y, CurrentPos.z),
									CurrentHeading
								)
							end
						end)
	...
	function FurnitureItems(house, Store, StoreIndex, Catagory)
		local Items = {
			{ unselectable = true, title = TranslateCap("store_title", Catagory), icon = "fas fa-store" },
	...
	function FurnitureCatagories(house, Store, StoreIndex)
		local Catagories = {
			{ unselectable = true, title = TranslateCap("store_title", Store), icon = "fas fa-store" },
			{ title = TranslateCap("back"), value = "go-back", icon = "fas fa-arrow-left" },
		}
	...
	function FurnitureEditSelect(propertyId)
		local Furni = {
			{ unselectable = true, title = TranslateCap("owned_furni"), icon = "far fa-edit" },
			{ title = TranslateCap("back"), value = "go-back", icon = "fas fa-arrow-left" },
		}
	...
	function FurnitureStores(house)
		local Options = {
			{ unselectable = true, title = TranslateCap("menu_stores"), icon = "fas fa-store" },
			{ title = TranslateCap("back"), value = "go-back", icon = "fas fa-arrow-left" },
		}
	...
	function FurnitureMenu(PropertyId)
		local Options = {
			{ unselectable = true, title = TranslateCap("furni_management"), icon = "fas fa-lamp" },
			{
				title = TranslateCap("menu_stores"),
				description = TranslateCap("menu_stores_desc"),
		...
		if #SpawnedFurniture > 0 then
			Options[#Options + 1] = {
				...
				description = TranslateCap("menu_reset_desc"),
				icon = "fas fa-trash",
				value = "reset",
			}
			Options[#Options + 1] = {
				title = TranslateCap("menu_edit"),
				description = TranslateCap("menu_edit_desc"),
				icon = "fas fa-archive",
				value = "edit",
			}
		end
