...
AddEventHandler("jsfour-idcard:open", function(ID, targetID, type)
    ...
    MySQL.Async.fetchAll("SELECT firstname, lastname, dateofbirth, sex, height FROM users WHERE identifier = @identifier", { ["@identifier"] = identifier }, function(user)
        if user[1] ~= nil then
            MySQL.Async.fetchAll("SELECT type FROM user_licenses WHERE owner = @identifier", { ["@identifier"] = identifier }, function(licenses)
                if type ~= nil then
                ...
                if show then
                    ...
                    TriggerClientEvent("esx:showNotification", _source, _U("no_license_type"))
                end
            end)
        end
    end)
