...
local NPCHasSpawnedTowable, NPCLastCancel, NPCHasBeenNextToTowable, NPCTargetDeleterZone =
	false, GetGameTimer() - 5 * 60000, false, false
...
function SelectRandomTowable()
	...
	for k, v in pairs(Config.Zones) do
		if
			v.Pos.x == Config.Towables[index].x
			and v.Pos.y == Config.Towables[index].y
			and v.Pos.z == Config.Towables[index].z
		then
			return k
		end
	end
		...
		if element.value == "vehicle_list" then
			if Config.EnableSocietyOwnedVehicles then
				...
				if
					...
					and (
						ESX.PlayerData.job.grade_name == "boss"
						or ESX.PlayerData.job.grade_name == "chief"
						or ESX.PlayerData.job.grade_name == "experimente"
					)
				then
					elements2[#elements2 + 1] = {
						icon = "fas fa-truck",
...
function OpenMechanicCraftMenu()
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= "recrue" then
		local elements = {
			...
			{ icon = "fas fa-gear", title = _U("repair_kit"), value = "fix_kit" },
		...
		if element.value == "billing" then
			local elements2 = {
				{ unselectable = true, icon = "fas fa-scroll", title = element.title },
				{
					...
					inputMin = 1,
					inputMax = 250000,
					inputPlaceholder = "Amount to bill..",
				},
							...
							if vehicle ~= targetVehicle then
								...
								if NPCOnJob then
									if NPCTargetTowable == targetVehicle then
										...
										Blips["NPCDelivery"] = AddBlipForCoord(
...
RegisterCommand("mechanicjob", function()
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == "mechanic" then
		if NPCOnJob then
			...
			if
				IsPedInAnyVehicle(playerPed, false) and IsVehicleModel(GetVehiclePedIsIn(playerPed, false), `flatbed`)
			then
				StartNPCJob()
			else
