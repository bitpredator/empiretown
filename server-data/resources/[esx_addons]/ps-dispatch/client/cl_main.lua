...
function zoneChance(type, zoneMod, street)
    ...
    elseif nearbyPeds < 1 then
        if Config.Debug then
            print(("^1[%s] Nobody is nearby to send a report^7"):format(type))
        end
        return false
    end
    if zoneMod == nil then
    ...
    if sum > 1 then
        ...
        if Config.Debug then
            print(("^2[%s] %s (%s) - %s nearby peds^7"):format(type, zone, chance, nearbyPeds))
        end
        sendit = true
    end
    print(("^1[%s] %s (%s) - %s nearby peds^7"):format(type, zone, chance, nearbyPeds))
...
function vehicleData(vehicle)
    ...
    if doorCount == 2 then
        doorCount = _U("two_door")
    elseif doorCount == 3 then
        doorCount = _U("three_door")
    elseif doorCount == 4 then
        doorCount = _U("four_door")
    else
        doorCount = ""
    end
...
function GetPedGender()
    local gender = "Male"
    if Functions[Config.Core].GetGender(PlayerData) == 1 then
        gender = "Female"
    end
    return gender
end
...
RegisterNetEvent("ps-dispatch:client:AddCallBlip", function(coords, data, blipId)
    if IsValidJob(data.recipientList) and CheckOnDuty() then
        ...
        CreateThread(function()
            ...
            if data.blipColour then
                colour = data.blipColour
            end
            if data.blipScale then
