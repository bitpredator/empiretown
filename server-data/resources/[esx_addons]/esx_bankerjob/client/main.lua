...
function OpenBankActionsMenu()
	local elements = {
		{unselectable = true, icon = "fas fa-bank", title = _U("bank")},
...
function OpenCustomersMenu()
	ESX.TriggerServerCallback('esx_bankerjob:getCustomers', function(customers)
		local elements = {
			{unselectable = true, icon = "fas fa-users", title = _U('customer')}
		}
		...
		ESX.OpenContext("right", elements, function(menu,element)
			local elements2 = {
				...
				{icon = "fas fa-wallet", title = _U('deposit'), value = "deposit"},
				{icon = "fas fa-wallet", title = _U('withdraw'), value = "withdraw"},
			}
			ESX.OpenContext("right", elements2, function(menu2,element2)
				local customer = element.data
				if element2.value == "deposit" then
					local elements = {
						{unselectable = true, icon = "fas fa-scroll", title = _U('amount')},
...
function CreateBillingDialog()
	local elements = {
		{unselectable = true, icon = "fas fa-scroll", title = _U('bill_amount')},
		{title = "Amount", input = true, inputType = "number", inputMin = 1, inputMax = 250000, inputPlaceholder = "Amount to bill.."},
		{icon = "fas fa-check-double", title = "Confirm", val = "confirm"}
	}
