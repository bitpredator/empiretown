...
if Config.FavKeybindEnabled then
    RegisterCommand("emotefav", function()
        FavKeybind()
    end, false)
...
function AddEmoteMenu(menu)
    ...
    if Config.SharedEmotesEnabled then
        sharemenu = _menuPool:AddSubMenu(submenu, Translate("shareemotes"), Translate("shareemotesinfo"), true, true)
    ...
    if not Config.SqlKeybinding then
        ...
        keyinfo = NativeUI.CreateItem(Translate("keybinds"), Translate("keybindsinfo") .. " /emotebind [~y~num4-9~w~] [~g~emotename~w~]")
    ...
    if Config.SharedEmotesEnabled then
        for a, b in pairsByKeys(RP.Shared) do
            ...
            if otheremotename == nil then
                ...
                shareitem = NativeUI.CreateItem(z, "/nearby (~g~" .. a .. "~w~) " .. Translate("makenearby") .. " (~y~" .. otheremotename .. "~w~)")
            end
    ...
    if not Config.SqlKeybinding then
        ...
        for emoteName, emoteLabel in pairsByKeys(favEmotes) do
            favemoteitem = NativeUI.CreateItem(emoteLabel, Translate("set") .. emoteLabel .. Translate("setboundemote"))
        ...
        if input ~= nil then
            ...
            if #results > 0 then
                ...
                searchMenu.OnItemSelect = function(sender, item, index)
                    ...
                    if data == Translate("sharedanceemotes") then
                        return
                    end
