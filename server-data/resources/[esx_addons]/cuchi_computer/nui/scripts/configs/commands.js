...
const CommandsList = {
	...
	'start': {
		...
		'action': (args) => {
			if (args.length > 0) {
				const appSpecified = args[0].replace('.exe', '');
				const openResult = OpenApp(appSpecified);
				if (openResult != 2) {AddConsoleLine('start ' + args[0], GetLocale(openResult == 1 ? 'cmd_started' : 'cmd_start_error').replace('{1}', appSpecified));}
				else {AddConsoleLine('start ' + args[0], GetLocale('cmd_start_already').replace('{1}', appSpecified));}
			}
			else {AddConsoleLine('start', GetLocale('cmd_start_not_specified'));}
		},
	},
	'taskkill': {
	...
	'ip-tracer': {
		...
		'action': (args) => {
			if (args.length > 0) {
				const ipOK = /^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/.test(args[0]);
				if (ipOK) {
					fetch(`https://${GetParentResourceName()}/ipTracer`,
						...
						if (data == 'DISCONNECTED') {AddConsoleLine('ip-tracer ' + args[0], GetLocale('cmd_iptracer_disconnected').replace('{1}', args[0]));}
						else {
			...
			else {AddConsoleLine('ip-tracer', GetLocale('cmd_iptracer_not_specified'));}
		},
	},
	'netscan': {
	...
	'connect': {
		...
		'action': (args) => {
			if (args.length > 0) {
				const specifiedDomain = args[0];
				const appExists = appAddresses[specifiedDomain] != undefined;
				if (appExists) {
					const appAddr = document.getElementById('addresses-addresse');
				...
				else {AddConsoleLine('connect ' + specifiedDomain, GetLocale('cmd_connect_unknown'));}
			}
			else {AddConsoleLine('connect', GetLocale('cmd_connect_not_specified'));}
		},
	},
	'detect': {
			...
			if (args.length > 0) {
				if (args[0] === '-ip') {
					fetch(`https://${GetParentResourceName()}/dataHeist`,
						...
						if (!data) {AddConsoleLine('detect ' + args[0], GetLocale('cmd_detect_none_detected'));}
						else {
				...
				else {AddConsoleLine('detect ' + args[0], GetLocale('cmd_detect_arg'));}
			}
			else {AddConsoleLine('detect', GetLocale('cmd_detect_arg'));}
		},
	},
	'scan': {
		...
		'action': (args) => {
			if (args.length > 0) {
				...
				for (const arg of args) {
					...
					if (arg in neededArgs) {nextArg = arg;}
				};
				...
				if (neededArgs['-ports'] !== '') {
					fetch(`https://${GetParentResourceName()}/dataHeist`,
						...
						if (!data) {AddConsoleLine('scan ' + (args.join(' ')), GetLocale('cmd_scan_wrong'));}
						else {
				...
				else {AddConsoleLine('scan ' + (args.join(' ')), GetLocale('cmd_scan_arg'));}
			}
			else {AddConsoleLine('scan', GetLocale('cmd_scan_arg'));}
		},
	},
	'infiltrate': {
		...
		'action': (args) => {
			if (args.length > 0) {
				...
				else {AddConsoleLine('infiltrate ' + (args.join(' ')), GetLocale('cmd_infiltrate_arg'));}
			}
			else {AddConsoleLine('infiltrate', GetLocale('cmd_infiltrate_arg'));}
		},
	},
	'breach': {
			...
			if (args.length > 0) {
				...
				if (neededArgs['-ip'] !== '' && neededArgs['-port'] !== '') {
					fetch(`https://${GetParentResourceName()}/dataHeist`,
						...
						else if (data === 'no') {AddConsoleLine('breach ' + (args.join(' ')), GetLocale('cmd_breach_no'));}
						else if (data === 'delay') {AddConsoleLine('breach ' + (args.join(' ')), GetLocale('cmd_breach_delay'));}
						else {
				...
				else {AddConsoleLine('breach ' + (args.join(' ')), GetLocale('cmd_breach_arg'));}
			}
			else {AddConsoleLine('breach', GetLocale('cmd_breach_arg'));}
		},
	},
};
