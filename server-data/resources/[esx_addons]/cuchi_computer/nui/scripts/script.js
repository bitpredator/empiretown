...
window.addEventListener('message', (event) => {
	if (event.data.type === 'show') {
		...
		let ComputerType = (event.data.laptop ? GetLocale('info_laptop') : GetLocale('info_desktop')).toLowerCase();
		AddInformation(GetLocale('info_type'), ComputerType);

		const mailDomain = event.data.mailDomain;
				...
				fetch(`https://${GetParentResourceName()}/getApplicationsData`,
					...
					for (const [appName, appData] of Object.entries(data)) {
						if (Applications[appName].usable && !Applications[appName].hide) {
							if (appName == 'market') {
								const marketConfig = event.data.market;
							...
							const container = document.getElementById(appName + '-container');
							container.innerHTML = '';
...
document.addEventListener('DOMContentLoaded', () => {
	...
	const localSelection = document.getElementById('locale-selection');
	...
	document.getElementById('exit').onclick = () => {
		...
		if (exitBtn.getAttribute('validation') === '1') {return;}
	...
	apps.forEach(app => {
		if (!Applications[app].hide) {document.getElementById(app).onclick = () => OpenApp(app);}
	...
	if (Applications['mail'].usable && !Applications['mail'].hide) {
		...
		const signUpBtn = document.getElementById('mail-signup-signup');
		signUpBtn.onclick = () => {
			document.getElementById('mail-signup').style.display = 'none';
			document.getElementById('mail-loader').style.display = 'block';

			const signupError = document.getElementById('mail-signup-error');
			signupError.style.display = 'none';
			signUpBtn.disabled = true;
			const pwdElem = document.getElementById('mail-signup-password');
			const pwdConfirmationElem = document.getElementById('mail-signup-password-confirmation');
			...
			fetch(`https://${GetParentResourceName()}/appAction`,
				...
				if (data == 'OK') {
					const connectionInfo = document.getElementById('mail-connection-info');
		...
		document.getElementById('mail-connection-signin').onclick = () => {
			document.getElementById('mail-connection').style.display = 'none';
			document.getElementById('mail-loader').style.display = 'block';
			const usernameInput = document.getElementById('mail-connection-username');
			const passwordInput = document.getElementById('mail-connection-password');
			fetch(`https://${GetParentResourceName()}/appAction`,
				{
					method: 'POST',
					body: JSON.stringify({
				...
				if (data == 'OK') {
					...
					const saveInputs = document.getElementById('mail-connection-save').checked;
					if (saveInputs) {
					...
					else {localStorage.removeItem('mail');}
		...
		const usernameSignUpInput = document.getElementById('mail-signup-username');
		usernameSignUpInput.oninput = () => {
			const previewElem = document.getElementById('mail-signup-preview');
		...
		// In app buttons
		const mailSignoutBtn = document.getElementById('mail-signout');
		mailSignoutBtn.onclick = () => setupMail();

		const mailCreateBtn = document.getElementById('mail-create');
		...
		document.getElementById('mail-creator-send').onclick = () => {
			...
			if (answeringTo) {
				const mailsData = mailsList[answeringTo];
				const firstMail = mailsData[mailsData.length - 1];
				const creatorTextInput = document.getElementById('mail-creator-text');
			...
			const creatorToInput = document.getElementById('mail-creator-to');
			const creatorObjectInput = document.getElementById('mail-creator-object');
			const creatorTextInput = document.getElementById('mail-creator-text');
			fetch(`https://${GetParentResourceName()}/appAction`,
				{
					method: 'POST',
				...
				const creator = document.getElementById('mail-creator');
		...
		const mailRefreshBtn = document.getElementById('mail-refresh');
...
const OpenApp = (appName, msgBox) => {
	...
	const elem = document.getElementById('app-' + appName);
	if (!elem) {
	...
	if (!taskbarIcon) {
		wasOpen = false;
		const taskbar = document.getElementById('left');
...
const CloseApp = (appName, callback) => {
	...
	if (taskbarIcon) {document.getElementById('left').removeChild(taskbarIcon);}
	else {wasRunning = false;}

	if (callback) callback();

	const isMsgBox = appName.split('_')[0] == 'msgbox';
	if (isMsgBox) {
		if (app) {app.remove();}

		const inAppsIndex = apps.indexOf(appName);
		if (inAppsIndex >= 0) {apps.splice(inAppsIndex, 1);}
	}
...
const UnfocusAllApp = () => {
	...
	while (focused.length > 0) {focused[0].classList.remove('app-active');}
};
...
const MakeElementDraggable = (element) => {
	...
	const movable = document.getElementById(element.id + '-title');
	...
	movable.onmousedown = (e) => {
		...
		document.onmousemove = (e) => {
			...
			if (newTop >= 0 && newTop <= (window.innerHeight - 0.10 * window.innerHeight)) {element.style.top = newTop + 'px';}
...
const OnConsoleCommand = (e, consoleInput) => {
	if (e.key === 'Enter') {
		...
		else {AddConsoleLine(consoleInput.value, GetLocale('cmd_unknown'));}

		if (consoleInput.value != '') {cmdHistory.push(consoleInput.value);}
		cmdHistIndex = 0;
	}
	else if (e.key === 'ArrowUp') {
		e.preventDefault();
		const newIndex = cmdHistory.length - (cmdHistIndex + 1);
		if (newIndex > 0) cmdHistIndex += 1;

		const cmd = cmdHistory[newIndex > 0 ? newIndex : 0];
		consoleInput.value = cmd || '';
		const cursorPosition = cmd ? cmd.length : 0;
		consoleInput.setSelectionRange(cursorPosition, cursorPosition);
	}
	else if (e.key === 'ArrowDown') {
		e.preventDefault();
		const newIndex = cmdHistory.length - 1 - (cmdHistIndex - 1);
		if (newIndex < cmdHistory.length) cmdHistIndex -= 1;

		const cmd = cmdHistory[newIndex < cmdHistory.length ? newIndex : cmdHistory.length];
		consoleInput.value = cmd || '';
		const cursorPosition = cmd ? cmd.length : 0;
		consoleInput.setSelectionRange(cursorPosition, cursorPosition);
	}
		...
		if (nbStartingBy === 1) {
			...
			const cmdLength = lastStartingBy.length;
...
const AddConsoleLine = (command, text) => {
	...
	const oldInput = document.getElementById('console-input');
	const enteredCommand = document.createElement('p');
...
const AddInformation = (label, value) => {
	const informations = document.getElementById('informations-text');
...
const MessageBox = (type, title, message, buttons, onClose, onMinimize) => {
	...
	const appName = 'msgbox_' + type + '_' + msgId;
...
const ShutdownComputer = (exitBtn, forced) => {
	...
	if (exitBtn) {exitBtn.removeAttribute('validation');}
	...
	apps.forEach(app => {
		...
		const appTextElement = document.getElementById(app + '-text');
		if (appTextElement) {
...
const editTheme = (themeData) => {
	const oldTheme = document.getElementById('theme-' + localStorage.getItem('--main-color'));
	if (oldTheme) {
		oldTheme.innerHTML = '';
...
const setupMail = () => {
	...
	if (mailString) {mailConnection = JSON.parse(mailString);}
...
const refreshMails = (callback) => {
	fetch(`https://${GetParentResourceName()}/appAction`,
		...
		for (const [key, mails] of Object.entries(mailsList)) {
			const date = new Date(mails[0].timestamp * 1000);
			const dateString = date.toLocaleTimeString(dateFormat) + ' ' + date.toLocaleDateString(dateFormat);
			const newElement = document.createElement('div');
			...
			newElement.onclick = () => {
				...
				const readerContainer = document.getElementById('mail-reader-container');
				document.getElementById('mail-container').style.display = 'none';
