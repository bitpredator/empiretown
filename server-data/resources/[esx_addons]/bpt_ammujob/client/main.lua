	...
	ESX.TriggerServerCallback('bpt_ammujob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_ammu', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('bpt_ammujob:getOtherPlayerData', function(playerData)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerServerEvent('bpt_ammujob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))

			TriggerServerEvent('bpt_ammujob:removeLicense', GetPlayerServerId(player), data.current.type)
...
function OpenWeaponComponentShop(components, weaponName, parentShop)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons_components', {
		...
		if data.current.hasComponent then
			...
			ESX.TriggerServerCallback('bpt_ammujob:buyWeapon', function(bought)
				if bought then
...
AddEventHandler('esx:setJob', function(job)
	...
	if job.name == 'ammu' then
		Wait(1000)
		TriggerServerEvent('bpt_ammujob:forceBlip')
	end
end)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ammu' and ESX.PlayerData.job.name == dispatchNumber then
...
ESX.RegisterInput("ammu:interact", "(BPT AMMU) Interact", "keyboard", "E", function()
...
ESX.RegisterInput("ammu:quickactions", "(BPT AMMUJOB) Quick Actions", "keyboard", "F6", function()
...
RegisterNetEvent('bpt_ammujob:updateBlip')
AddEventHandler('bpt_ammujob:updateBlip', function()
...
if ESX.PlayerLoaded and ESX.PlayerData.job == 'ammu' then
