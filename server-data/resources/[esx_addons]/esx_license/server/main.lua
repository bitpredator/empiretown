...
local function GetLicenses(identifier, cb)
    MySQL.query("SELECT user_licenses.type, licenses.label FROM user_licenses LEFT JOIN licenses ON user_licenses.type = licenses.type WHERE owner = ?", { identifier }, function(result)
        if cb then
            cb(result)
        end
    end)
