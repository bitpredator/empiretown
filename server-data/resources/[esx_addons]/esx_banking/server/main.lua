	...
	if amount == nil or (not typeData.pincode and amount <= 0) then
		...
		if typeData.transfer then
			TriggerClientEvent(
				...
				source,
				_U(string.format("%s_money", key), amount, typeData.transfer.playerId),
				"success"
			)
		else
			TriggerClientEvent(
				"esx:showNotification",
				source,
				_U(string.format("%s_money", key), typeData.pincode and (string.format("%04d", amount)) or amount),
				"success"
			)
		end
...
ESX.RegisterServerCallback("esx_banking:getPlayerData", function(source, cb)
	...
	local transactionHistory = MySQL.Sync.fetchAll(
		"SELECT * FROM banking WHERE identifier = ? AND time > ? ORDER BY time DESC LIMIT 10",
		{ identifier, weekAgo }
	)
	local playerData = {
...
ESX.RegisterServerCallback("esx_banking:checkPincode", function(source, cb, inputPincode)
	local xPlayer = ESX.GetPlayerFromId(source)
	local identifier = xPlayer.getIdentifier()
	local pincode = MySQL.Sync.fetchScalar(
		"SELECT COUNT(1) AS pincode FROM users WHERE identifier = ? AND pincode = ?",
...
BANK = {
	...
	Transfer = function(xTarget, xPlayer, amount)
		...
		TriggerClientEvent(
			"esx:showNotification",
			xTarget.source,
			_U("receive_transfer", amount, xPlayer.source),
			"success"
		)
