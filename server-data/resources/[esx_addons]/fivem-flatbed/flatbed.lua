    ...
    while true do
        ...
        if veh and DoesEntityExist(veh) and GetEntityModel(veh) == GetHashKey("flatbed3") and NetworkHasControlOfEntity(veh) then
            ...
            if not DecorExistOn(veh, "flatbed3_bed") or DecorGetInt(veh, "flatbed3_bed") == 0 then
                ...
                local bed = CreateObject("inm_flatbed_base", pos, 1, 0, false, false, false)
                Log("GENERATING BED")
            ...
            if bedNet ~= 0 then
                ...
                elseif state == 3 then
                    ...
                    if not lowered then
                        ...
                        DetachEntity(bed, false, false)
                        AttachEntityToEntity(bed, veh, GetEntityBoneIndexByName(veh, "chassis"), offsetPos, offsetRot, 0, 0, 1, 0, false, true, true, true, 0, false)
                        DecorSetBool(veh, "flatbed3_lowered", true)
                        lowered = true
                    end
                        ...
                        if IsControlJustPressed(0, 47) then
                            if attached then
                                ...
                                if newCar then
                                    ...
                                    while not NetworkHasControlOfEntity(newCar) do
                                        Wait(0)
                                    end
