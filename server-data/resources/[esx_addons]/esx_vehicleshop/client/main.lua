	...
	ESX.TriggerServerCallback("esx_vehicleshop:getCommercialVehicles", function(vehicles)
		...
		for _, v in ipairs(vehicles) do
			...
			table.insert(elements, {
				label = ('%s [<span style="color:orange;">%s</span>]'):format(
					vehicleLabel,
...
function OpenShopMenu()
	...
	for i = 1, #Categories, 1 do
		...
		for j = 1, #categoryVehicles, 1 do
			...
			table.insert(
				options,
				('%s <span style="color:green;">%s</span>'):format(
					vehicle.name,
...
function OpenPopVehicleMenu()
	ESX.TriggerServerCallback("esx_vehicleshop:getCommercialVehicles", function(vehicles)
		...
		for k, v in ipairs(vehicles) do
			...
			table.insert(elements, {
				label = ('%s [MSRP <span style="color:green;">%s</span>]'):format(
					vehicleLabel,
...
AddEventHandler("esx_vehicleshop:hasEnteredMarker", function(zone)
	...
	elseif
		...
		and ESX.PlayerData.job.name == "cardealer"
