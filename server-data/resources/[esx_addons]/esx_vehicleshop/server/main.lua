            ...
            if id then
                MySQL.update("DELETE FROM cardealer_vehicles WHERE id = ?", { id }, function(rowsChanged)
                    if rowsChanged == 1 then
                        ...
                        MySQL.insert("INSERT INTO vehicle_sold (client, model, plate, soldby, date) VALUES (?, ?, ?, ?, ?)", {
                            xTarget.getName(),
                            label,
                            vehicleProps.plate,
                            xPlayer.getName(),
        ...
        MySQL.single("SELECT id, price FROM cardealer_vehicles WHERE vehicle = ?", { vehicle }, function(result)
            if result then
                MySQL.update("DELETE FROM cardealer_vehicles WHERE id = ?", { result.id }, function(rowsChanged)
                    if rowsChanged == 1 then
                        MySQL.insert("INSERT INTO rented_vehicles (vehicle, plate, player_name, base_price, rent_price, owner) VALUES (?, ?, ?, ?, ?, ?)", { vehicle, plate, xTarget.getName(), result.price, rentPrice, xTarget.identifier }, function(id)
...
ESX.RegisterServerCallback("esx_vehicleshop:buyVehicle", function(source, cb, model, plate)
    ...
    if modelPrice and xPlayer.getMoney() >= modelPrice then
        ...
        MySQL.insert("INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (?, ?, ?)", { xPlayer.identifier, plate, json.encode({ model = joaat(model), plate = plate }) }, function(rowsChanged)
...
ESX.RegisterServerCallback("esx_vehicleshop:giveBackVehicle", function(_, cb, plate)
    MySQL.single("SELECT base_price, vehicle FROM rented_vehicles WHERE plate = ?", { plate }, function(result)
        if result then
            ...
            MySQL.update("DELETE FROM rented_vehicles WHERE plate = ?", { plate }, function(rowsChanged)
...
ESX.RegisterServerCallback("esx_vehicleshop:retrieveJobVehicles", function(source, cb, type)
    ...
    MySQL.query("SELECT * FROM owned_vehicles WHERE owner = ? AND type = ? AND job = ?", { xPlayer.identifier, type, xPlayer.job.name }, function(result)
        cb(result)
    end)
end)
...
AddEventHandler("esx_vehicleshop:setJobVehicleState", function(plate, state)
    ...
    MySQL.update("UPDATE owned_vehicles SET `stored` = ? WHERE plate = ? AND job = ?", { state, plate, xPlayer.job.name }, function(rowsChanged)
...
function PayRent()
    ...
    MySQL.query("SELECT rented_vehicles.owner, rented_vehicles.rent_price, rented_vehicles.plate, users.accounts FROM rented_vehicles LEFT JOIN users ON rented_vehicles.owner = users.identifier", {}, function(rentals)
        local owners = {}
        for i = 1, #rentals do
