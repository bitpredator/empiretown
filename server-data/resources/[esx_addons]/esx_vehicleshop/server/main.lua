						...
						MySQL.insert(
							...
							function(id)
								xPlayer.showNotification(
									TranslateCap("vehicle_set_owned", vehicleProps.plate, xTarget.getName())
								)
				...
				MySQL.update("DELETE FROM cardealer_vehicles WHERE id = ?", { result.id }, function(rowsChanged)
					if rowsChanged == 1 then
						MySQL.insert(
							...
							{ vehicle, plate, xTarget.getName(), result.price, rentPrice, xTarget.identifier },
							function(id)
	...
	if xPlayer.job.name == "cardealer" then
		MySQL.single("SELECT id, price FROM cardealer_vehicles WHERE vehicle = ?", { vehicleModel }, function(result)
			if result then
				...
				MySQL.update("DELETE FROM cardealer_vehicles WHERE id = ?", { id }, function(rowsChanged)
					if rowsChanged == 1 then
						TriggerEvent("bpt_addonaccount:getSharedAccount", "society_cardealer", function(account)
							...
							xPlayer.showNotification(
								TranslateCap("vehicle_sold_for", vehicleLabel, ESX.Math.GroupDigits(price))
							)
						end)
					end
				end)
				...
				print(
					("[^3WARNING^7] Player ^5%s^7 Attempted To Sell Invalid Vehicle - ^5%s^7!"):format(
						source,
						vehicleModel
					)
				)
	...
	MySQL.single("SELECT base_price, vehicle FROM rented_vehicles WHERE plate = ?", { plate }, function(result)
		if result then
			...
			MySQL.update("DELETE FROM rented_vehicles WHERE plate = ?", { plate }, function(rowsChanged)
				MySQL.insert(
					"INSERT INTO cardealer_vehicles (vehicle, price) VALUES (?, ?)",
					{ result.vehicle, result.base_price }
				)
