...
CreateThread(function()
    Wait(1000) -- ritardo iniziale per sicurezza

    local oxItems = exports.ox_inventory:Items()
    local seedItem = Config.Items.Seed
    local tries = 60
    local foundItem = false

    while not foundItem and tries > 0 do
        if oxItems and oxItems[seedItem] then
            foundItem = true

            print("✅ Oggetto '" .. seedItem .. "' trovato in ox_inventory.")
            ...
            ESX.RegisterUsableItem(seedItem, function(source)
                ...
                if now > last + (Config.ActionTime / 1000) then
                    if HasItem(source, seedItem) then
            ...
            print("⏳ Tentativo di caricamento ox_inventory (" .. (61 - tries) .. "/60)...")
            Wait(1000)
            tries = tries - 1
        end
    end
    ...
    if not foundItem then
        print("❌ ERRORE: Oggetto '" .. seedItem .. "' non trovato in ox_inventory!")
    end
end)
