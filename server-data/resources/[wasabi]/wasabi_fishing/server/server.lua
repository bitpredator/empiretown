	...
	if xPole > 0 and xBait > 0 then
		...
		if Config.OldESX then
			...
			if limitItem.limit == -1 or (limitItem.count + 1) <= limitItem.limit then
				...
				TriggerClientEvent(
					"wasabi_fishing:notify",
					source,
					Strings.fish_success,
					string.format(Strings.fish_success_desc, awardLabel),
					"success"
				)
			else
				TriggerClientEvent(
					"wasabi_fishing:notify",
					source,
					Strings.cannot_carry,
					Strings.cannot_carry_desc,
					"error"
				)
			end
		else
			...
			for i = 1, #Config.fish do
				...
				if xPlayer.getInventoryItem(Config.fish[i].item).count > 0 then
					...
					TriggerClientEvent(
						...
						source,
						Strings.sold_for,
						(Strings.sold_for_desc):format(
							xPlayer.getInventoryItem(Config.fish[i].item).count,
							xPlayer.getInventoryItem(Config.fish[i].item).label,
							addCommas(rewardAmount)
						),
						"success"
					)
